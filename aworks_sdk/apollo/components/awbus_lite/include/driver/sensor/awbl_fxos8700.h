...
#define AWBL_FXOS8700_NAME                  "awbl_fxos8700"
...
typedef aw_err_t (*I2C_SendFunc_t)(uint8_t deviceAddress, uint32_t subAddress, uint8_t subaddressSize, uint32_t txBuff);
typedef aw_err_t (*I2C_ReceiveFunc_t)(
 ...
 */
aw_err_t FXOS_Init(fxos_handle_t *fxos_handle, fxos_config_t *config);
 ...
 * @return kStatus_Success if success or kStatus_Fail if error.
 */
aw_err_t FXOS_ReadSensorData(fxos_handle_t *fxos_handle, fxos_data_t *sensorData);
 ...
 * @return kStatus_Success if success or kStatus_Fail if error.
 */
aw_err_t FXOS_WriteReg(fxos_handle_t *handle, uint8_t reg, uint8_t val);
 ...
 * @return kStatus_Success if success or kStatus_Fail if error.
 */
aw_err_t FXOS_ReadReg(fxos_handle_t *handle, uint8_t reg, uint8_t *val, uint8_t bytesNumber);
...
typedef struct awbl_sensor_fxos8700_devinfo {

    uint8_t     start_chn_id;
    uint8_t     slave_addr;
...
}awbl_sensor_fxos8700_devinfo_t;



typedef struct awbl_sensor_fxos8700_dev {
    ...
    void                       *cb_parg;
    AW_SEMB_DECL(               isr_semb);
    aw_task_id_t                isr_tsk;

}awbl_sensor_fxos8700_dev_t;
