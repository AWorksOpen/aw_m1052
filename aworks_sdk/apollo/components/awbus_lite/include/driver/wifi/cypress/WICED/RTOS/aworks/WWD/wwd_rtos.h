...
#define RTOS_HIGHEST_PRIORITY            AW_TASK_SYS_PRIORITY(4)//(configMAX_PRIORITIES-1)
...
#ifdef DEBUG
#define WWD_THREAD_STACK_SIZE        (1024*8)   /* Stack checking requires a larger stack */
#else /* ifdef DEBUG */
#define WWD_THREAD_STACK_SIZE        (1400 + 1400)
#endif /* ifdef DEBUG */
#else /* if WWD_LOGGING_STDOUT_ENABLE */
#define WWD_THREAD_STACK_SIZE        (1400 + 1024 + 1400 + 4096) /* WWD_LOG uses printf and requires a minimum of 4K stack */
...
typedef struct host_semaphore_type {
    AW_SEMC_DECL(sem);
} host_semaphore_type_t;

typedef struct host_thread_type {
    AW_TASK_DECL(wwd_task, WWD_THREAD_STACK_SIZE);
} host_thread_type_t;

typedef void host_queue_type_t;
