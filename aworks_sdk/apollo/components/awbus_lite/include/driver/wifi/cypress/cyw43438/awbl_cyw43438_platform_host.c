 ...
 * 使用本服务需要包含头文件
 * \code
 * #include "awbl_cyw43438_platform_host.c"
 * \endcode
 *
 * \internal
...
#include "driver/wifi/cypress/cyw43438/private/awbl_cyw43438_sdio.h"
...
/******************************************************************************
   HOST
 ******************************************************************************/
extern struct awbl_sdio_host              *__gp_cyw43438_sdio_host;
extern awbl_sdio_cyw43438_wlan_dev_info_t *__gp_cyw43438_sdio_info;
extern struct awbl_dev                    *__gp_cyw43438_sdio_dev;
...
wwd_result_t host_platform_init (void)
{
     ...
     * 因为设备在bus已经枚举完成
     */
    aw_gpio_pin_cfg(__gp_cyw43438_sdio_info->base.reset_pin,
...
#if 0
    aw_gpio_set(__gp_cyw43438_sdio_info->base.reset_pin,
...
{
    return awbl_43438_sdio_enable_int();
}
                                         ...
                                         wwd_interface_t interface)
{
    ...
    if (__gp_cyw43438_sdio_dev == NULL) {
        return;
    }
    awbl_wiced_eth_pkt_processor(__gp_cyw43438_sdio_dev, buffer, interface);
...
wwd_result_t host_platform_sdio_transfer (wwd_bus_transfer_direction_t direction,
                                          ...
                                          uint8_t                      *data)
{
    return awbl_43438_sdio_transfer(direction, function, address, data_size, data);
}
...
void host_platform_nic_link_up (uint8_t interface)
{
    ...
    awbl_wiced_nic_set_link_up(__gp_cyw43438_sdio_dev, interface);
}
...
void host_platform_nic_link_down (uint8_t interface)
{
    ...
    awbl_wiced_nic_set_link_down(__gp_cyw43438_sdio_dev, interface);
}
...
void host_platform_firmware_size (uint32_t *size_out)
{
    ...
    __gp_cyw43438_sdio_info->base.resource_hdls.fw.pfn_size(size_out);
}
...
void host_platform_cfg_nvram_size (uint32_t *size_out)
{
    ...
    __gp_cyw43438_sdio_info->base.resource_hdls.nv.pfn_size(size_out);
}
...
void host_platform_clm_blob_size (uint32_t *size_out)
{
    ...
    __gp_cyw43438_sdio_info->base.resource_hdls.clm.pfn_size(size_out);
}
                                  ...
                                  uint32_t *size_out)
{
    ...
    __gp_cyw43438_sdio_info->base.resource_hdls.fw.pfn_read(offset,
                                                            buffer,
                                                            size_in,
                                                            size_out);
}
                                   ...
                                   uint32_t *size_out)
{
    ...
    __gp_cyw43438_sdio_info->base.resource_hdls.nv.pfn_read(offset,
                                                            buffer,
                                                            size_in,
                                                            size_out);
}
                                  ...
                                  uint32_t *size_out)
{
    ...
    __gp_cyw43438_sdio_info->base.resource_hdls.clm.pfn_read(offset,
                                                             buffer,
                                                             size_in,
                                                             size_out);
}
