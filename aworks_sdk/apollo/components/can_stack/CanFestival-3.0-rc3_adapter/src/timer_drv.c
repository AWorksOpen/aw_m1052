...
#define __TIMER_INTERRUPT_FREQ_100KHZ 10000 /**< \brief 100KHZ中断频率 */
#define __TIMER_100US_UINT            100   /**< \brief 100us单位 */
...
typedef struct __timer_handle {
    ...
    /** \brief 协议栈互斥锁 */
    AW_MUTEX_DECL           (cop_mutex);    
    
    /** \brief 硬件定时器句柄 */
    aw_hwtimer_handle_t      p_hwtimer;     
    
    /** \brief 硬件定时器中断延迟作业 */
    struct aw_isr_defer_job  isr_defer_job; 
} __timer_handle_t;
