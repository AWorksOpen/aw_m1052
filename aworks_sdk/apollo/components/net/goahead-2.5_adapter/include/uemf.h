...
#if (defined (SCOV5) || defined (VXWORKS) || defined (LINUX) || \
     defined (LYNX) || defined (MACOSX))
#ifndef O_BINARY
#define O_BINARY         0
#endif /* O_BINARY */
...
#define SYM_MAX                (512)
#define XML_MAX             4096    /**< \brief Maximum size for tags/tokens */
#define BUF_MAX             4096    /**< \brief General sanity check for bufs */
#ifndef LINE_MAX
#define LINE_MAX   2048    /**< \brief General sanity check for a single line */
#endif /* LINE_MAX */
#define FMT_STATIC_MAX        256  /**< \brief Maximum for fmtStatic calls */
...
/** \brief fix page.c line 79 => sbuf->mtime = s.st_mtime; */
...
#elif (defined (LYNX) || defined (LINUX) || defined (MACOSX) || \
       defined (SOLARIS))
#define gchdir        chdir
...
#ifndef FNAMESIZE
#define FNAMESIZE            254         /**< \brief Max length of file names */
#endif /* FNAMESIZE */

#define E_MAX_ERROR          4096
#define URL_MAX              4096
#define E_MAX_REQUEST        2048        /**< \brief Request safeguard max */

/*
 * Error types
 */
#define    E_ASSERT       0x1       /**< \brief Assertion error */
#define    E_LOG          0x2       /**< \brief Log error to log file */
#define    E_USER         0x3       /**< \brief Error that must be displayed */
...
typedef struct {
    ...
    unsigned int    allocated    : 8;       /**< \brief String was balloced */
} value_t;
 ...
 */
typedef struct {
    unsigned char    *buf;               /**< \brief Holding buffer for data */
    unsigned char    *servp;             /**< \brief Pointer to start of data */
    unsigned char    *endp;              /**< \brief Pointer to end of data */
    unsigned char    *endbuf;            /**< \brief Pointer to end of buffer */
    int                buflen;           /**< \brief Length of ring queue */
    int                maxsize;          /**< \brief Maximum size */
    int                increment;        /**< \brief Growth increment */
} ringq_t;
...
typedef struct {
    union {
        void    *next;                /**< \brief Pointer to next in q */
        int        size;              /**< \brief Actual requested size */
    } u;
    int            flags;             /**< \brief Per block allocation flags */
} bType;

#define B_SHIFT           4             /**< \brief Convert size to class */
#define B_ROUND           ((1 << (B_SHIFT)) - 1)
#define B_MAX_CLASS       13            /**< \brief Maximum class number + 1 */
#define B_MALLOCED        0x80000000    /**< \brief Block was malloced */
#define B_DEFAULT_MEM    (64 * 1024)    /**< \brief Default memory allocation */
#define B_MAX_FILES      (512)          /**< \brief Maximum number of files */
#define B_FILL_CHAR      (0x77)         /**< \brief Fill byte for buffers */
#define B_FILL_WORD      (0x77777777)   /**< \brief Fill word for buffers */
#define B_MAX_BLOCKS     (64 * 1024)    /**< \brief Maximum allocated blocks */

/*
 *    Flags. The integrity value is used as an arbitrary value to fill the flags.
 */
#define B_INTEGRITY       0x8124000  /**< \brief Integrity value */
#define B_INTEGRITY_MASK  0xFFFF000  /**< \brief Integrity mask */
#define B_USE_MALLOC      0x1        /**< \brief Okay to use malloc if required */
#define B_USER_BUF        0x2        /**< \brief User supplied buffer for mem */
...
typedef struct sym_t {
    struct sym_t    *forw;           /**< \brief Pointer to next hash list */
    value_t          name;           /**< \brief Name of symbol */
    value_t          content;        /**< \brief Value of symbol */
    int              arg;            /**< \brief Parameter value */
} sym_t;

typedef int sym_fd_t;                /**< \brief Returned by symOpen */

/*
 *    Script engines
 */
#define EMF_SCRIPT_JSCRIPT            0        /**< \brief javascript */
#define EMF_SCRIPT_TCL                1        /**< \brief tcl */
#define EMF_SCRIPT_EJSCRIPT           2        /**< \brief Ejscript */
...
#define SOCKET_EOF           0x1     /**< \brief Seen end of file */
#define SOCKET_CONNECTIN     0x2     /**< \brief Connect in progress */
#define SOCKET_BROADCAST     0x4     /**< \brief Broadcast mode */

#define SOCKET_PENDING       0x8   /**< \brief Message pending on this socket */

#define SOCKET_FLUSHING      0x10    /**< \brief Background flushing */
#define SOCKET_DATAGRAM      0x20    /**< \brief Use datagrams */
#define SOCKET_ASYNC         0x40    /**< \brief Use async connect */
#define SOCKET_BLOCK         0x80    /**< \brief Use blocking I/O */
#define SOCKET_LISTENING     0x100   /**< \brief Socket is server listener */
#define SOCKET_CLOSING       0x200   /**< \brief Socket is closing */
#define SOCKET_CONNRESET     0x400   /**< \brief Socket connection was reset */
#define SOCKET_MYOWNBUFFERS  0x800   /**< \brief Not using inBuf/outBuf ringq */

#define SOCKET_PORT_MAX      0xffff  /**< \brief Max Port size */

/*
 *    Socket error values
 */
#define SOCKET_WOULDBLOCK    1        /**< \brief Socket would block on I/O */
#define SOCKET_RESET         2        /**< \brief Socket has been reset */
#define SOCKET_NETDOWN       3        /**< \brief Network is down */
#define SOCKET_AGAIN         4        /**< \brief Issue the request again */
#define SOCKET_INTR          5        /**< \brief Call was interrupted */
#define SOCKET_INVAL         6        /**< \brief Invalid */

/*
 *    Handler event masks
 */
#define SOCKET_READABLE       0x2        /**< \brief Make socket readable */ 
#define SOCKET_WRITABLE       0x4        /**< \brief Make socket writable */
#define SOCKET_EXCEPTION      0x8        /**< \brief Interested in exceptions */
#define EMF_SOCKET_MESSAGE   (WM_USER+13)

#define WEBS_MAX_REQUEST      2048        /**< \brief Request safeguard max */

#ifdef LITTLEFOOT
#define SOCKET_BUFSIZ         510        /**< \brief Underlying buffer size */
#else
#define SOCKET_BUFSIZ         1024       /**< \brief Underlying buffer size */
#endif /* LITTLEFOOT */
...
typedef int        (*socketAccept_t)(int sid, char *ipaddr, int port, 
                    int listenSid);
typedef struct {
    char            host[64];        /**< \brief Host name */
    ringq_t         inBuf;           /**< \brief Input ring queue */
    ringq_t         outBuf;          /**< \brief Output ring queue */
    ringq_t         lineBuf;         /**< \brief Line ring queue */
    socketAccept_t  accept;          /**< \brief Accept handler */
    socketHandler_t handler;         /**< \brief User I/O handler */
    void            *handler_data;   /**< \brief User handler data */
    int              handlerMask;    /**< \brief Handler events of interest */
    int              sid;            /**< \brief Index into socket[] */
    int              port;           /**< \brief Port to listen on */
    int              flags;          /**< \brief Current state flags */
    int              sock;           /**< \brief Actual socket handle */
    int              fileHandle;     /**< \brief ID of the file handler */
    int              interestEvents; /**< \brief Mask of events to watch for */
    int              currentEvents;  /**< \brief Mask of ready events (FD_xx) */
    int              selectEvents;   /**< \brief Events being selected */
    int              saveMask;       /**< \brief saved Mask for socketFlush */
    int              error;          /**< \brief Last error */
} socket_t;
...
/** \brief Added by LohCT to support binary file upload. */
#define bmemdup(B_ARGS, p, s) bmemdupNoBalloc(p, s)     
...
/** \brief Added by LohCT to support binary file upload. */
#define bmemdup(B_ARGS, p, s) bmemdup(p, s)             
...
extern char_t   *bmemdup(B_ARGS_DEC, char_t *s, int size);      
...
/* Added by LohCT to support binary file upload. */
extern char_t   *xmemstr(char_t *memBlk, char_t *find, int memSize);       
