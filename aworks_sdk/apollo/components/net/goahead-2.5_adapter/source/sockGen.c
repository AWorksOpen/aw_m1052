...
/********************************** Includes **********************************/
#include    "uemf.h"
#define NOT_HAVE_SYS_TYPES_H
/* TI compiler donot have sys/types.h */
#ifndef NOT_HAVE_SYS_TYPES_H
#include        <sys/types.h>
#else
    #define fd_mask int
    #define NBBY 8
...
#if ((defined (WIN) || defined (CE)) && \
     (!defined (LITTLEFOOT) && !defined  (WEBS)))
    if ( !(sp->flags & SOCKET_BLOCK)
...
/******************************************************************************/
/*
 *    Return AW_TRUE if there is a socket with an event ready to process,
 */
 ...
 */
    if (sp->flags & SOCKET_BLOCK) {
        ...
        lwip_fcntl(
            sp->sock, 
            F_SETFL, 
            lwip_fcntl(sp->sock, F_GETFL, O_NONBLOCK) & ~O_NONBLOCK);/* changed */
#endif
