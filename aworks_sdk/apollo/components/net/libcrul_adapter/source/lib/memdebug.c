...
{
  if(curl_dbg_logfile < 0) {
    if(logname && *logname)
      curl_dbg_logfile = aw_open(logname, O_RDWR, 0777);
    else
      curl_dbg_logfile = stderr;
#ifdef MEMDEBUG_LOG_SYNC
    /* Flush the log file after every line so the log isn't lost in a crash */
    if(curl_dbg_logfile > -1)
...
int curl_dbg_fopen(const char *file, const char *mode,
                    int line, const char *source)
{
  int res = aw_open(file, mode, 0777);
...
int curl_dbg_fdopen(int filedes, const char *mode,
                      int line, const char *source)
{
  int res = aw_open(filedes, mode, 0777);
  if(source)
...
int curl_dbg_fclose(int file, int line, const char *source)
{
  int res;
