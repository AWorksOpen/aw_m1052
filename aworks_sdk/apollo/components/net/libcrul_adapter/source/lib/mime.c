                           ...
                           const struct_stat *stat_buf)
{
  ...
  file = aw_open(name, O_RDONLY, 0777); /* VMS */
  if(file < 0)
    return 0;
  ...
  ret_stat = 1;
  while(ret_stat > 0) {
    ret_stat = aw_read(file, buffer, sizeof(buffer),);
    if(ret_stat != 0)
      count += ret_stat;
  }
  aw_close(file);
 ...
 *
 */
static int vmsfopenread(const char *file, const char *mode)
{
  struct_stat statbuf;
  int result;

  result = aw_stat(file, &statbuf);

  switch(statbuf.st_fab_rfm) {
  case FAB$C_VAR:
  case FAB$C_VFC:
  case FAB$C_STMCR:
    return aw_open(file, O_RDONLY, 777); /* VMS */
    break;
  default:
    return aw_open(file, O_RDONLY, 777);
  }
}
...
static int mime_open_file(curl_mimepart *part)
{
  ...
  if(part->fd > -1)
    return 0;
  part->fd = fopen_read(part->data, O_RDONLY, 0777);
  return (part->fd > -1)? 0: -1;
}
                             ...
                             void *instream)
{
  ...
  return (size_t)aw_read(part->fd, buffer, size * nitems);
}
...
static int mime_file_seek(void *instream, curl_off_t offset, int whence)
{
  ...
  if(whence == SEEK_SET && !offset && (part->fd < 0))
    return CURL_SEEKFUNC_OK;   /* Not open: implicitly already at BOF. */

  if(mime_open_file(part))
    return CURL_SEEKFUNC_FAIL;

  return aw_lseek(part->fd, (long) offset, whence)?
  ...
  if(part->datasize != (curl_off_t) -1 &&
    ...
    case MIMEKIND_FILE:
       if (part->fd > -1) {
           struct aw_stat stat;
           size_t off;
           aw_fstat(part->fd, &stat);
           aw_ioctl(part->fd, AW_FIOWHERE, &off);
           if (off == stat.st_size) {
               break;  /* At EOF. */
           }
       }
    ...
    default:
      if(part->readfunc) {
        ...
        sz = part->readfunc(buffer, bufsize, 1, part->arg);
      }
      break;
    }
  }
  ...
  while(bufsize) {
    ...
    case MIMESTATE_CONTENT:
      ...
      case 0:
        ...
        if(part->kind == MIMEKIND_FILE && (part->fd > -1)) {
          aw_close(part->fd);
          part->fd = -1;
        }
        /* FALLTHROUGH */
