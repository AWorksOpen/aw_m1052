...
Curl_tls_keylog_open(void)
{
  ...
  if(keylog_file_fd < 0) {
    ...
    if(keylog_file_name) {
      keylog_file_fd = aw_open(keylog_file_name, O_RDWR|O_APPEND, 777);
      if(keylog_file_fd> -1) {
        if(setvbuf(keylog_file_fd, NULL, _IOLBF, 4096))
        {
          aw_close(keylog_file_fd);
          keylog_file_fd = -1;
        }
      }
