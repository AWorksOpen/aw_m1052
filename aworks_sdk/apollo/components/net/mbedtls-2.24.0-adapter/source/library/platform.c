...
int mbedtls_platform_std_nv_seed_read( unsigned char *buf, size_t buf_len )
{
    ...
    if( ( file = aw_open( MBEDTLS_PLATFORM_STD_NV_SEED_FILE, O_RDONLY, 0777 ) ) < 0 )
        return( -1 );

    if( ( n = aw_read( file, buf, buf_len ) ) != buf_len )
    {
        aw_close( file );
...
int mbedtls_platform_std_nv_seed_write( unsigned char *buf, size_t buf_len )
{
    ...
    if( ( file = aw_open( MBEDTLS_PLATFORM_STD_NV_SEED_FILE, O_WRONLY, 0777 ) ) < 0 )
        return -1;

    if( ( n = aw_write(file, buf, buf_len ) ) != buf_len )
    {
        aw_close( file );
        return -1;
    }
