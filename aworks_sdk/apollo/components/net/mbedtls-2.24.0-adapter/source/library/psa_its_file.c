                                       ...
                                       int *p_stream )
{
    ...
    *p_stream = aw_open( filename, O_RDONLY, 0777);
    if( *p_stream < 0 )
        return( PSA_ERROR_DOES_NOT_EXIST );

    n = aw_read(*p_stream, &header, sizeof( header ));
    if( n != sizeof( header ) )
                          ...
                          size_t *p_data_length )
{
    ...
    {
        if( aw_lseek( stream, LONG_MAX, SEEK_CUR ) != 0 )
            goto exit;
        data_offset -= LONG_MAX;
    }
#endif
    if( aw_lseek( stream, data_offset, SEEK_CUR ) != 0 )
        goto exit;
    n = aw_read(stream, p_data, data_length );
    if( n != data_length )
        goto exit;
    status = PSA_SUCCESS;
                          ...
                          psa_storage_create_flags_t create_flags )
{
    ...
    psa_its_fill_filename( uid, filename );
    stream = aw_open( PSA_ITS_STORAGE_TEMP, O_RDWR, 0777);
    if( stream < 0 )
        goto exit;

    status = PSA_ERROR_INSUFFICIENT_STORAGE;
    n = aw_write(stream , &header, sizeof( header ) );
    if( n != sizeof( header ) )
        goto exit;
    if( data_length != 0 )
    {
        n = aw_write(stream, p_data, data_length );
        if( n != data_length )
            goto exit;
    }
    status = PSA_SUCCESS;
...
psa_status_t psa_its_remove( psa_storage_uid_t uid )
{
    ...
    int stream;
    psa_its_fill_filename( uid, filename );
    stream = aw_open( filename, O_RDONLY, 0777);
    if( stream < 0)
        return( PSA_ERROR_DOES_NOT_EXIST );
    aw_close( stream );
