...
}rtk_armv7m_jlink_rtos_info_t;

aw_const rtk_armv7m_jlink_rtos_info_t   rtk_armv7m_jlink_rtos_info = {
    ...
    rtk_task_option_t  option)
{
    ...
    *(--stk) = 4;                           /* r4 */
    ...
    if (option & ENUM_RTK_TASK_OPTION_UNPRIVILEGED) {
        ...
        flag |= ARMV7M_CONTEXT_FLAGS_PRIVILEGED;
    }
#endif
#if __RTK_ARMV7M_CONTEXT_USE_FLAGS
...
static void __fpu_init(void)
{
    ...
    AW_REG_BITS_SET32(&ARMV7M_SCB->CPACR,20,4,0xF);
    rtk_arm_barrier_dsb();

    /* 使能lazy context save */
    writel(0xC0000000,&ARMV7M_FPU->FPCCR);
    rtk_arm_barrier_dsb();
#else
    /* 否则就禁用对CP10和CP11的访问，以避免出现意料不到的问题 */
    AW_REG_BITS_SET32(&ARMV7M_SCB->CPACR,20,4,0);
    rtk_arm_barrier_dsb();
#endif
}
...
void rtk_arch_init()
{
    ...
    /* 设置亚优先级为最低位0 */
    armv7m_nvic_set_prio_group(0);

    /* 检查优先级位数配置 */
    writeb(0xFF,&ARMV7M_NVIC->IP[0]);
    prio = readb(&ARMV7M_NVIC->IP[0]);
    count = 0;
    for (i = 7;i >=0 ; i --) {
        if (prio & (1 << i)) {
            count ++;
    ...
    if (count < CONFIG_RTK_ARMV7M_NVIC_PRIO_BITS) {
        while (1);
    }


    prio_count = armv7m_nvic_get_priority_count();
    /* PENDSV异常用于任务切换，将PENDSV异常的优先级设为最低 */
    prio = prio_count - 1;
    armv7m_nvic_set_priority(ARMv7m_PendSV_IRQn,prio);
    armv7m_nvic_set_priority(ARMv7m_DebugMonitor_IRQn,prio);
    /* SVC 异常通常用于实现系统调用，它不能被负责任务切换的pendsv打断 */
    /* 但可以被其它中断打断 */
    /* 所以其优先级和pendsv一样 */
    armv7m_nvic_set_priority(ARMv7m_SVCall_IRQn,prio);
    /* systick优先级要高于svc，但可低于其它中断 */
    prio -= 1;
    armv7m_nvic_set_priority(ARMv7m_SysTick_IRQn,prio);

    /* usage ,memmanage,bus这三种fault 优先级配置为0 */
    armv7m_nvic_set_priority(ARMv7m_MemManagement_IRQn,0);
    armv7m_nvic_set_priority(ARMv7m_BusFault_IRQn,0);
    armv7m_nvic_set_priority(ARMv7m_UsageFault_IRQn,0);

    prio -= 1;
    /* 其它中断优先级配置为比systick大1 */
    count =( (ARMV7M_SCnSCB->ICTR & 0xF) + 1 ) * 32;
    for (i = 0;i<count;i++) {
         armv7m_nvic_set_priority(i,prio);
         armv7m_nvic_irq_disable(i);
    }


    /* 使能usage ,memmanage,bus这三种fault */
    AW_REG_BIT_SET32(&ARMV7M_SCB->SHCSR,18);
    AW_REG_BIT_SET32(&ARMV7M_SCB->SHCSR,17);
    AW_REG_BIT_SET32(&ARMV7M_SCB->SHCSR,16);

    /* 使能除零错误 */
    AW_REG_BIT_SET32(&ARMV7M_SCB->CCR,4);
    rtk_arm_barrier_dsb();
