...
static void backtrace_cur_tasks_data_abort(uint32_t  abort_lr,
                                           void     *frame_base,
                                           void     *lr)
{
    int             count=0;
...
static void show_data_abort_status(int status)
{
    ...
    if(AW_BIT_ISSET(status,13)) {
        kprintf("Abort caused by a cache maintenance operation.\n");
    }
    else {
        kprintf("Abort not caused by a cache maintenance operation.\n");
    }

    if(AW_BIT_ISSET(status,12)) {
        kprintf("External abort marked as DECERR.\n");
    }
    else {
        kprintf("External abort marked as SLVERR.\n");
    }

    if(AW_BIT_ISSET(status,11)) {
        kprintf("Abort caused by a write access.\n");
    }
    else {
        kprintf("Abort caused by a read access.\n");
    }
