...
#ifndef __ARMV8M_NO_CACHE /* 当前CPU带有cache */
...
void armv8m_dcache_clean_by_mva(uint32_t mva);
void armv8m_dcache_invalidate_by_mva(uint32_t mva);
int armv8m_cache_get_cache_level_count(void);
int armv8m_dcache_get_line_size(void);
...
void armv8m_dcache_clean (uint32_t buffer, uint32_t size)
{
    ...
    if (0 == armv8m_cache_get_cache_level_count()) {
        return;
    }

    cache_line_size = armv8m_dcache_get_line_size();

    ptr = buffer & ~(cache_line_size - 1); /* todo: assert the buffer */

    while (ptr < buffer + size) {
        armv8m_dcache_clean_by_mva(ptr);
        ptr += cache_line_size;
    }
}
...
void armv8m_dcache_invalidate (uint32_t buffer, uint32_t size)
{
    ...
    while (ptr < buffer + size) {
        armv8m_dcache_invalidate_by_mva(ptr);
        ptr += cache_line_size;
    }
}
