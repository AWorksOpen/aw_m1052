...
/**
 * \file
 * \brief ARMv8-M exception library
 *
 * \internal
...
/******************************************************************************/
void armv8m_exc_nmi_handler (void)
{
...
static void show_memmanage_fault_info(void)
{
    ...
    mfsr = AW_REG_BITS_GET32(&ARMV8M_SCB->CFSR, 0, 8);
...
static void show_bus_fault_info(void)
{
    ...
    bfsr = AW_REG_BITS_GET32(&ARMV8M_SCB->CFSR, 8, 8);
...
static void show_usage_fault_info(void)
{
    ...
    ufsr = AW_REG_BITS_GET32(&ARMV8M_SCB->CFSR, 16, 16);
...
static const struct armv8m_fault __g_fault[] = {
        {"unknown",NULL},
        {"unknown",NULL},
        {"unknown",NULL},
...
static void armv8m_show_fault_context(fault_error_context_t *p)
{
...
void armv8m_show_fault_error(void)
{
    volatile int                not_exit = 1;
    fault_error_context_t      *p_fatal_context = &__g_fault_err_context;
    int                         val;
    const struct armv8m_fault   *p_fault_info = NULL;

    val = readl(&ARMV8M_SCB->ICSR);
    val &= 0x1FF;
    ...
    armv8m_show_fault_context(p_fatal_context);
    while(not_exit);
}

/******************************************************************************/
void armv8m_exc_systick_handler (void)
{
    rtk_tick_down_counter_announce();
}

/******************************************************************************/
void armv8m_exc_eint_handler (void)
{
...
void armv8m_exc_nvic_int_handler (void)
{
    RTK_ENTER_INT_CONTEXT();
    armv8m_nvic_gen_int_handler();
    RTK_EXIT_INT_CONTEXT();
}
