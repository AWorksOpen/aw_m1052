...
{
    return ( 1<< CONFIG_RTK_ARMV8M_NVIC_PRIO_BITS);
}
...
void armv8m_nvic_set_priority(ARMv8m_IRQn_t irqn, uint32_t prio)
{
    ...
    data = (uint8_t)(( prio << (8 - CONFIG_RTK_ARMV8M_NVIC_PRIO_BITS)) & 0xFF);

    if(irqn < 0) {
        irqn = 16 - (0 - irqn) - 4;
        /* set Priority for Cortex-M System Interrupts */
        writeb(data, &ARMV8M_SCB->SHPR[irqn]);
    }
    else {
        /* set Priority for device specific Interrupts  */
        writeb(data, &ARMV8M_NVIC->IPR[irqn]);
    }
}
...
uint32_t armv8m_nvic_get_priority(ARMv8m_IRQn_t irqn)
{
    ...
    if(irqn < 0) {
        ...
        data = readb(&ARMV8M_SCB->SHPR[irqn]);
    }
    else {
        /* get Priority for device specific Interrupts  */
        data = readb(&ARMV8M_NVIC->IPR[irqn]);
    }

    prio = ((uint32_t)data) >> (8 - CONFIG_RTK_ARMV8M_NVIC_PRIO_BITS);
...
void armv8m_nvic_set_prio_group(uint32_t prio_grouping)
{
    ...
    /* ?????ио???? */
    reg_value = readl(&ARMV8M_SCB->AIRCR);
    reg_value &= (1<< 15);
    ...
    reg_value |= (0x05FA << 16);
    writel(reg_value, &ARMV8M_SCB->AIRCR);
    rtk_arm_barrier_dsb();
}
...
uint32_t armv8m_nvic_get_prio_group(void)
{
    ...
    reg_value = AW_REG_BITS_GET32(&ARMV8M_SCB->AIRCR, 8, 3);
...
void armv8m_nvic_irq_enable(ARMv8m_IRQn_t irqn)
{
    ...
    if (irqn >= 0) {
        ...
        p_addr = (uint32_t *)&(ARMV8M_NVIC->ISER[mask]);
        mask = ( 1<< (irqn & 0x1F));
        writel(mask, p_addr);
        rtk_arm_barrier_dsb();
    }
...
void armv8m_nvic_irq_disable(ARMv8m_IRQn_t irqn)
{
    ...
    if (irqn >= 0) {
        mask = ((uint32_t)irqn) >> 5;
        p_addr = (uint32_t *)&(ARMV8M_NVIC->ICER[mask]);
        mask = ( 1<< (irqn & 0x1F));
        writel(mask, p_addr);
        rtk_arm_barrier_dsb();
    }
...
void armv8m_nvic_irq_set_pending(ARMv8m_IRQn_t irqn)
{
    ...
    if (irqn >= 0) {
        mask = ((uint32_t)irqn) >> 5;
        p_addr = (uint32_t *)&(ARMV8M_NVIC->ISPR[mask]);
        mask = ( 1<< (irqn & 0x1F));
        writel(mask, p_addr);
        rtk_arm_barrier_dsb();
    }
...
void armv8m_nvic_irq_clr_pending(ARMv8m_IRQn_t irqn)
{
    ...
    if (irqn >= 0) {
        mask = ((uint32_t)irqn) >> 5;
        p_addr = (uint32_t *)&(ARMV8M_NVIC->ICPR[mask]);
        mask = ( 1<< (irqn & 0x1F));
        writel(mask, p_addr);
        rtk_arm_barrier_dsb();
    }
        ...
        uint32_t arg_count)
{
    ...
    irq_count =( (ARMV8M_SCnSCB->ICTR & 0xF) + 1 ) * 32;
    ...
    p_org_vector_start = (uint32_t *)readl(&ARMV8M_SCB->VTOR);
    ...
    rtk_arm_barrier_dsb();
    writel((uint32_t)p_vector_start, &ARMV8M_SCB->VTOR);
    rtk_arm_barrier_dsb();
    rtk_arm_barrier_isb();
        ...
        void *p_arg)
{
    ...
    max_isr_count = ( (ARMV8M_SCnSCB->ICTR & 0xF) + 1 ) * 32;
        ...
        pfn_armv8m_nvic_isr_t isr_func)
{
    ...
    RTK_ENTER_CRITICAL(old);
    _g_p_vector_start[irqn + 16] = (uint32_t)armv8m_nvic_gen_int_handler;
...
ARMv8m_IRQn_t armv8m_nvic_get_active_irq(void)
{
    ...
    active_irq = (readl(&ARMV8M_SCB->ICSR) & 0x1FF);

    irq = (int)(int32_t)(active_irq - 16);

    return (ARMv8m_IRQn_t)irq;
}
