...
/**
 * \file
 * \brief IMX1010 Flexspi NOR Flash通用接口
 *
 * 使用本服务需要包含头文件
 * \code
 * #include imx1010_nor_flash.h
 * \endcode
 *
 * \par 简单示例
 * \code
...
#ifndef __IMX1010_NOR_FLASH_H
#define __IMX1010_NOR_FLASH_H
 ...
 */
aw_err_t  imx1010_nor_flash_read (uint8_t *p_buf,
                                  uint32_t offset,
                                  uint32_t nbytes);
 ...
 *       在对flash进行编程前必须调用imx1010_nor_flash_sector_erase函数对指定区域进行擦除
 */
 ...
 *       在使用之前需调用imx1010_nor_flash_ir_erase对待写入的行进行擦除
 */
