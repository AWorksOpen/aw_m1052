...
#define __FLEXSPI_FLASH_DEVINFO_DECL(p_info, p_nor_dev) \
        awbl_imx1010_flexspi_devinfo_t *p_info = \
        (awbl_imx1010_flexspi_devinfo_t *)AWBL_DEVINFO_GET(p_nor_dev)
                          ...
                          struct aw_mtd_erase_info  *instr)
{
    ...
    awbl_imx1010_flexspi_devinfo_t *p_flash_devinfo = NULL;
    ...
    p_flash_devinfo = (awbl_imx1010_flexspi_devinfo_t *)AWBL_DEVINFO_GET(&p_fmtd->p_flash->super);
    ...
    while (len) {
        ...
        /* 擦除扇区 */
        ret = imx1010_nor_flash_sector_erase(addr);
                         ...
                         void                *p_buf)
{
    ...
    /* 读取数据 */
    ret = imx1010_nor_flash_read(p_buf, from, len);
                          ...
                          const void           *p_buf)
{
    ...
    while (length) {
        ...
        ret = imx1010_nor_flash_page_program(ptr, addr, bytes);
        if (ret != AW_OK) {
            goto __exit;
        }
...
aw_err_t awbl_flexspi_flash_mtd_init (struct awbl_imx1010_flexspi_dev *p_flash,
                                      void                            *p_obj,
                                      const void                      *p_info)
{
