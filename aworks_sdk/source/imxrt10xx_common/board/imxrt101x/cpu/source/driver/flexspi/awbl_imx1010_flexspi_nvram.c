                                           ...
                                           int               len)
{
    ...
    for(i = 0; i < p_devinfo->nvram_info.seglst_count; i++) {
        if ( ((p_seg->unit) == unit) && (strcmp(p_seg->p_name, p_name) == 0) ) {
            ...
            /* 读取数据 */
            ret = imx1010_nor_flash_read((uint8_t *)p_buf, offset, copylen);
    ...
    for (i = 0; i < p_devinfo->nvram_info.seglst_count; i++) {
        if ((p_seg->unit == unit) && (strcmp(p_seg->p_name, p_name) == 0)) {
            ...
            for (j = 0; j < copylen;) {
                ...
                /* 读取一扇区数据 */
                ret = imx1010_nor_flash_read((uint8_t *)(p_devinfo->nvram_info.p_blk_buf),
                                             sector_addr_start,
                                             sector_size);
                ...
                /* 擦除扇区 */
                imx1010_nor_flash_sector_erase(sector_addr_start);
                ...
                for (n = 0; n < sector_page_cnt; n++) {
                    ...
                    ret = imx1010_nor_flash_page_program((uint8_t *)(p_devinfo->nvram_info.p_blk_buf + n * page_size),
