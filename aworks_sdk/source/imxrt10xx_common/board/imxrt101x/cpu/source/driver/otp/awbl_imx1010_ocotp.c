...
#include "awbl_plb.h"
#include "driver/otp/awbl_imx1010_ocotp.h"
#include "driver/otp/imx1010_ocotp_regs.h"
...
#define __IMX6UL_OCOTP_DEVINFO_DECL(p_devinfo, p_dev) \
    struct awbl_imx1010_ocotp_devinfo *p_devinfo = \
        (struct awbl_imx1010_ocotp_devinfo *)AWBL_DEVINFO_GET(p_dev)
...
*******************************************************************************/
aw_local void __imx1010_ocotp_inst_init1(awbl_dev_t *p_dev);
aw_local void __imx1010_ocotp_inst_init2(awbl_dev_t *p_dev);
aw_local void __imx1010_ocotp_inst_connect(awbl_dev_t *p_dev);

aw_local aw_err_t __imx1010_ocotp_nvram_get(struct awbl_dev *p_dev,
                                            char            *p_name,
                                            int              unit,
                                            char            *p_buf,
                                            int              offset,
...
aw_local aw_err_t __imx1010_ocotp_nvram_set(struct awbl_dev *p_dev,
                                            char            *p_name,
                                            int              unit,
                                            char            *p_buf,
                                            int              offset,
...
aw_local aw_const struct awbl_drvfuncs __g_imx1010_ocotp_drvfuncs = {
	__imx1010_ocotp_inst_init1,
	__imx1010_ocotp_inst_init2,
	__imx1010_ocotp_inst_connect
};
...
aw_local aw_const struct awbl_dev_method __g_imx1010_ocotp_dev_methods[] = {
    AWBL_METHOD(awbl_nvram_get, __imx1010_ocotp_nvram_get),
    AWBL_METHOD(awbl_nvram_set, __imx1010_ocotp_nvram_set),
    AWBL_METHOD_END
};

/** \brief driver registration (must defined as aw_const) */
aw_local aw_const awbl_plb_drvinfo_t __g_imx1010_ocotp_drv_registration = {
    {
        AWBL_VER_1,                           /* awb_ver */
        AWBL_BUSID_PLB,                       /* bus_id */
        AWBL_IMX1010_OCOTP_NAME,              /* p_drvname */
        &__g_imx1010_ocotp_drvfuncs,          /* p_busfuncs */
        &__g_imx1010_ocotp_dev_methods[0],    /* p_methods */
...
#define __IMX1010_OCOTP_TP_MAXSIZE_GET()      512
...
static aw_err_t Delay4Busy(imx1010_ocotp_regs_t *p_regs)
{
...
aw_local aw_err_t  __imx1010_ocotp_fuse_burn(struct awbl_imx1010_ocotp_dev *p_dev,
                                             uint32_t                       addr,
                                             uint32_t                       data)
{
	__IMX6UL_OCOTP_DEVINFO_DECL(p_devinfo, p_dev);
    ...
    uint32_t            reg_val = 0;
    int                 result = AW_OK;
    imx1010_ocotp_regs_t *p_regs;

    p_regs = (imx1010_ocotp_regs_t *)p_devinfo->regbase;
...
aw_local aw_err_t  __imx1010_ocotp_fuse_read(struct awbl_imx1010_ocotp_dev *p_dev,
                                               uint32_t                    addr,
                                               uint32_t                    *data)
{
    __IMX6UL_OCOTP_DEVINFO_DECL(p_devinfo, p_dev);
    imx1010_ocotp_regs_t      *p_regs = NULL;
    aw_err_t                   err = AW_OK;
    uint32_t                  reg_ctl = 0;
...
aw_local int8_t
__imx1010_ocotp_read_bytes_in_word (struct awbl_imx1010_ocotp_dev *p_dev,
                                    uint32_t                       addr,
                                    uint8_t                        start,
                                    uint8_t                        end,
                                    uint8_t                       *pbuf)
{
    ...
    ret = __imx1010_ocotp_fuse_read (p_dev, addr, &data_reg);
    if (ret < 0) {
        return ret;
    }
...
aw_local int8_t
__imx1010_ocotp_burn_bytes_in_word (struct awbl_imx1010_ocotp_dev *p_dev,
                                    uint32_t                       addr,
                                    uint8_t                        start,
                                    uint8_t                        end,
                                    uint8_t                       *pbuf)
{
    ...
    ret = __imx1010_ocotp_fuse_burn (p_dev, addr, data_reg);
    if (ret < 0) {
        return ret;
    }
 ...
 */
aw_local aw_err_t __imx1010_ocotp_rw (struct awbl_imx1010_ocotp_dev *p_dev,
                                      int                            start,
                                      uint8_t                       *p_buf,
                                      size_t                         len,
                                      aw_bool_t                      is_read)
{
    uint32_t maxsize = __IMX1010_OCOTP_TP_MAXSIZE_GET();
    ...
    while (len_tmp) {
        ...
        if (is_read) {
            ...
            nbytes = __imx1010_ocotp_read_bytes_in_word (p_dev, start, start % 4,
                                                         end, p_buf);
            ...
            nbytes = __imx1010_ocotp_burn_bytes_in_word (p_dev, start, start % 4,
                                                         end, p_buf);
            AW_INT_CPU_UNLOCK(lock);
        }
                                             ...
                                             int              len)
{
    ...
    for (i = 0; i < p_devinfo->seglst_count; i++) {
        if ((p_seg->unit == unit) && (strcmp (p_seg->p_name, p_name) == 0)) {
            ...
            ret = __imx1010_ocotp_rw ((struct awbl_imx1010_ocotp_dev *) p_dev,
...
{
    awbl_drv_register (
            (struct awbl_drvinfo *) &__g_imx1010_ocotp_drv_registration);
}
