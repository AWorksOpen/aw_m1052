...
#include "driver/sai/imx1010_sai_regs.h"
#include "driver/sai/awbl_imx1010_sai.h"
...
/* declare imx1010 sai infomation */
#define __SAI_DEV_DECL(p_this, p_dev) \
    struct awbl_imx1010_sai_dev *p_this = \
        (struct awbl_imx1010_sai_dev *)(p_dev)

#define __SAI_DEVINFO_DECL(p_devinfo, p_dev) \
    struct awbl_imx1010_sai_devinfo *p_devinfo = \
        (struct awbl_imx1010_sai_devinfo *)AWBL_DEVINFO_GET(p_dev)

#define __SAI_REGS_DECL(p_regs, p_devinfo) \
    struct imx1010_sai_regs *p_regs = \
        (struct imx1010_sai_regs *)(p_devinfo->regbase)
...
aw_local aw_err_t __sai_tx_reset (struct awbl_imx1010_sai_dev *p_this)
{
    volatile uint32_t           tmp;
...
void __imx1010_sai_tx_dma_callback (void *p_arg, unsigned int len)
{
 ...
 */
aw_local
aw_err_t __imx1010_sai_dai_sysclk_set (struct awsa_soc_dai *p_dai,
                                       uint32_t             clk_id,
                                       uint32_t             freq,
                                       int                  dir)
{
 ...
 */
aw_local
aw_err_t __imx1010_sai_dai_startup (struct awsa_soc_dai *p_dai)
{
 ...
 */
aw_local
aw_err_t __imx1010_sai_dai_shutdown (struct awsa_soc_dai *p_dai,
 ...
 */
aw_local
aw_err_t __imx1010_sai_dai_config (struct awsa_soc_dai *p_dai,
                                   awsa_pcm_stream_t    stream,
...
/* dai servopts */
aw_local struct awsa_soc_dai_servopts __g_imx1010_sai_dai_opts = {
    __imx1010_sai_dai_sysclk_set,
    NULL,
    NULL,
    NULL,
    NULL,
    __imx1010_sai_dai_startup,
    __imx1010_sai_dai_shutdown,
    __imx1010_sai_dai_config,
    NULL,
    NULL,
    NULL
};
...
/* dai driver */
aw_local struct awsa_soc_dai_driver __g_imx1010_sai_dai_driver = {
    "imx1010",
    &__g_imx1010_sai_dai_opts,
...
aw_local
aw_err_t __imx1010_sai_pcm_config (struct awsa_soc_platform *p_plfm,
                                   awsa_pcm_stream_t         stream,
                                   awsa_pcm_config_t        *p_config)
{
    __SAI_DEV_DECL(p_this, p_plfm->p_private_data);
...
aw_local
aw_err_t __imx1010_sai_dma_alloc (struct awsa_soc_platform  *p_plfm,
                                  void                     **p_dma_mem)
{
    ...
    /* SAI_NUM range: 0 ~ 2 */
    if (unit_id >= AWBL_IMX1010_SAI_NUM) {
        return AW_ERROR;
    }
    ...
    p_this->p_tx_dma_chan = imx1010_edma_alloc_chan(&data);
...
aw_local
aw_err_t __imx1010_sai_dma_free (struct awsa_soc_platform  *p_plfm,
                                 void                      *p_dma_mem)
{
    __SAI_DEV_DECL(p_this, p_plfm->p_private_data);
    ...
    if (p_this->p_tx_dma_chan != NULL) {
        imx1010_edma_free_chan(p_this->p_tx_dma_chan);
    }
...
/* dma prepare */
aw_local
aw_err_t __imx1010_sai_pcm_prepare (struct awsa_soc_platform *p_plfm)
{

    __SAI_DEV_DECL(p_this, p_plfm->p_private_data);
    __SAI_DEVINFO_DECL(p_devinfo, p_this);
    struct imx1010_sai_regs  *p_regs = (struct imx1010_sai_regs *)p_devinfo->regbase;
    aw_err_t ret = AW_OK;
   ...
   // aw_cache_flush(p_this->app_ptr, p_this->dma_size);
    ...
    ret = imx1010_edma_control(p_this->p_tx_dma_chan,
                               DMA_SLAVE_CONFIG,
                               (unsigned long)&cfg);
    if (ret) {
    ...
    /* 准备一个发送循环传输  */
    ret = imx1010_edma_prep_dma_cyclic(p_this->p_tx_dma_chan,
...
/* pcm trigger */
aw_local
aw_err_t __imx1010_sai_pcm_trigger (struct awsa_soc_platform  *p_plfm,
                                    int                        cmd)
{
    __SAI_DEV_DECL(p_this, p_plfm->p_private_data);
    __SAI_DEVINFO_DECL(p_devinfo, p_this);
    ...
    case AWSA_CORE_PCM_TRIGGER_START:
        ...
        ret = imx1010_edma_chan_start_async(
                  p_this->p_tx_dma_chan, __imx1010_sai_tx_dma_callback, p_this);
    ...
    case AWSA_CORE_PCM_TRIGGER_STOP:
        ...
        imx1010_edma_chan_stop(p_this->p_tx_dma_chan);
...
/* pcm pointer */
aw_local
awsa_pcm_uframes_t __imx1010_sai_pcm_pointer (struct awsa_soc_platform  *p_plfm)
{
    __SAI_DEV_DECL(p_this, p_plfm->p_private_data);
...
/* platform driver */
aw_local struct awsa_soc_platform_driver __g_imx1010_sai_plfm_driver = {
    NULL,
    NULL,
    NULL,
    NULL,
    NULL,
    __imx1010_sai_pcm_config,
    __imx1010_sai_dma_alloc,
    __imx1010_sai_dma_free,
    __imx1010_sai_pcm_prepare,
    __imx1010_sai_pcm_trigger,
    __imx1010_sai_pcm_pointer,
    NULL,
    NULL,
};
...
aw_local void __sai_inst_connect(awbl_dev_t *p_dev)
{
    ...
    awsa_soc_platform_register(&p_this->soc_plfm,
                               AWBL_IMX1010_SAI_NAME,
                               &__g_imx1010_sai_plfm_driver,
                               &p_this->plfm_dai,
                               &__g_imx1010_sai_dai_driver,
                               1,
                               (void *)p_this);
}
...
aw_local aw_const awbl_plb_drvinfo_t __g_sai_drv_registration = {
    {
        ...
        AWBL_IMX1010_SAI_NAME,                  /* p_drvname */
