...
#include "driver/timestamp/awbl_imx1010_timestamp.h"

#define __TIMESTAMP_DEV_DECL(p_this, p_dev)   \
          struct awbl_imx1010_timestamp_dev *p_this = \
          (struct awbl_imx1010_timestamp_dev *)p_dev

#define __TIMESTAMP_DEVINFO_DECL(p_devinfo, p_dev)   \
          struct awbl_imx1010_timestamp_devinfo *p_devinfo = \
          (struct awbl_imx1010_timestamp_devinfo *)AWBL_DEVINFO_GET(p_dev)
...
/* timestamp */
struct awbl_timestamp __g_imx1010_timestamp = {NULL, 0};
...
aw_local void __timestamp_inst_init2 (awbl_dev_t *p_dev)
{
    ...
    __g_imx1010_timestamp.fn_timestamp_get = __timestamp_get;

    if (p_devinfo->freq < 1) {
        __g_imx1010_timestamp.freq = 1;
    } else if (p_devinfo->freq > MAX_TIMESTAMP_FREQUENCY) {
        __g_imx1010_timestamp.freq = MAX_TIMESTAMP_FREQUENCY;
    } else {
        __g_imx1010_timestamp.freq = p_devinfo->freq;
    }
    
    /* 初始化时间戳记录变量 */
    __g_timestamp_record.sys_freq = aw_clk_rate_get(IMX1010_CLK_AHB_PODF) / __g_imx1010_timestamp.freq;
    if (__g_timestamp_record.sys_freq == 0) {
        __g_timestamp_record.sys_freq = 1;
    }
...
aw_local void __timestamp_inst_connect(awbl_dev_t *p_dev)
{
    ...
    awbl_timestamp_dev_register(&__g_imx1010_timestamp);
...
aw_local aw_const struct awbl_drvinfo __g_timestamp_drv_registration = {
    ...
    AWBL_IMX1010_TIMESTAMP_NAME,                /* p_drvname*/
