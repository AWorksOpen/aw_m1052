...
#ifndef __AWBL_IMX1050_TIMESTAMP_H_
#define __AWBL_IMX1050_TIMESTAMP_H_

#define AWBL_IMX1050_TIMESTAMP_NAME     "awbl_imx1050_timestamp"
...
/** \brief 设备配置信息 */
typedef struct awbl_imx1050_timestamp_devinfo {
    uint32_t        update_period;      /**< \brief 更新计数的周期时间（ms）. */
    uint32_t        freq;               /**< \brief 自定义的时钟频率. */

    /** \brief 指向平台初始化函数的指针 */
    void             (*pfn_plfm_init) (void);
} awbl_imx1050_timestamp_devinfo_t;

/** \brief 设备实例 */
typedef struct awbl_imx1050_timestamp_dev {
    awbl_dev_t      dev;                /**< \brief AWBus 设备实例结构体 */

    aw_timer_t      p_timer;            /**< \brief 软件定时器. */

} awbl_imx1050_timestamp_dev_t;
...
#endif /* __AWBL_IMX1050_TIMESTAMP_H_ */
