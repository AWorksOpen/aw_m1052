...
static void enet1_tx_ref_init(struct clk *hw)
{
    ...
    p_reg = (imx1050_ccm_regs_t *)IOMUXC_GPR_GPR1_REG_ADDR;
    val = readl(p_reg);
    if (val & (1 << 13)) {
        val &= ~(1 << 17);
        parent_id = IMX1050_CLK_ENET1_EX_TX_REF_IN;
    }
    else {
        val |= (1<< 17);
        parent_id = IMX1050_CLK_ENET1_REF;
    }
...
static aw_err_t enet1_tx_ref_set_parent(struct clk *hw,aw_clk_id_t parent_id)
{
    ...
    p_reg = (imx1050_ccm_regs_t *)IOMUXC_GPR_GPR1_REG_ADDR;
    val = readl(p_reg);
    if (IMX1050_CLK_ENET1_EX_TX_REF_IN == parent_id ) {
        val |= (1<<13);
        val &= ~(1 << 17);
    }
    else if (IMX1050_CLK_ENET1_REF == parent_id) {
        val &= ~(1<<13);
        val |= (1 << 17);
    }
    else {
...
struct clk * imx1050_clk_register_enet1_tx_ref(
