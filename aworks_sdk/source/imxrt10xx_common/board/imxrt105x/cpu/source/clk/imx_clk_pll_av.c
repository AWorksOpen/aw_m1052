...
struct clk * imx1050_clk_register_pll_audio(
...
static void pll_video_init(struct clk *hw)
{
    imx1050_ccm_analog_regs_t  *p_ccm_analog;
    ...
    // Audio post div,0x2 代表除1
    AW_REG_BITS_SET32(&p_ccm_analog->PLL_VIDEO,19,2,0x2);
    // Audio div,0x0代表除0
    AW_REG_BITS_SET32(&p_ccm_analog->MISC2,23,1,0x0);
    AW_REG_BITS_SET32(&p_ccm_analog->MISC2,15,1,0x0);
    // 使用osc作为bypass
    AW_REG_BITS_SET32(&p_ccm_analog->PLL_VIDEO,14,2,0x0);
    AW_REG_BIT_SET32(&p_ccm_analog->PLL_VIDEO,16);
}
...
static aw_clk_rate_t pll_video_recalc_rate(
        struct clk *hw,
        aw_clk_rate_t parent_rate)
{
    imx1050_ccm_analog_regs_t  *p_ccm_analog;
    ...
    p_ccm_analog = (imx1050_ccm_analog_regs_t *)IMX1050_CCM_ANALOG_BASE;
    return pll_av_recalc_rate(
            &p_ccm_analog->PLL_VIDEO,
            parent_rate,
            pll_video_get_video_div);
}



static aw_err_t pll_video_enable(struct clk *hw)
{
    imx1050_ccm_analog_regs_t  *p_ccm_analog;

    p_ccm_analog = (imx1050_ccm_analog_regs_t *)IMX1050_CCM_ANALOG_BASE;
    return pll_av_enable(&p_ccm_analog->PLL_VIDEO);
}

static aw_err_t pll_video_disable(struct clk *hw)
{
    imx1050_ccm_analog_regs_t  *p_ccm_analog;

    p_ccm_analog = (imx1050_ccm_analog_regs_t *)IMX1050_CCM_ANALOG_BASE;
    return pll_av_disable(&p_ccm_analog->PLL_VIDEO);
}
...
static aw_err_t pll_video_set_rate(
        struct clk *hw,
        aw_clk_rate_t rate,
        aw_clk_rate_t parent_rate)
{
    ...
    p_ccm_analog = (imx1050_ccm_analog_regs_t *)IMX1050_CCM_ANALOG_BASE;
    return pll_av_set_rate(&p_ccm_analog->PLL_VIDEO,
            rate,parent_rate,pll_video_set_video_div_val);
}

aw_const static struct clk_ops pll_video_ops = {
        .init = pll_video_init,
        .recalc_rate = pll_video_recalc_rate,
        .enable = pll_video_enable,
        .disable = pll_video_disable,
        .round_rate = pll_av_round_rate,
        .set_rate = pll_video_set_rate,
};

struct clk * imx1050_clk_register_pll_video(
        struct clk *p_pll_video,
        aw_clk_id_t clk_id,
        const char *name,
        aw_clk_id_t parent_id)
{
    return imx_clk_register(p_pll_video,clk_id,name,parent_id,&pll_video_ops);
}
