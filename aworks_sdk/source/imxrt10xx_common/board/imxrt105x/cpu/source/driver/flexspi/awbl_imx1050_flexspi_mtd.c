...
#define __FLEXSPI_FLASH_DEVINFO_DECL(p_info, p_nor_dev) \
        awbl_imx1050_flexspi_devinfo_t *p_info = \
        (awbl_imx1050_flexspi_devinfo_t *)AWBL_DEVINFO_GET(p_nor_dev)
                          ...
                          struct aw_mtd_erase_info  *instr)
{
    ...
    awbl_imx1050_flexspi_devinfo_t *p_flash_devinfo = NULL;
    ...
    p_flash_devinfo = (awbl_imx1050_flexspi_devinfo_t *)AWBL_DEVINFO_GET(&p_fmtd->p_flash->super);
    ...
    while (len) {
        ...
        /* 擦除扇区 */
        ret = awbl_flexspi_nor_flash_erase(addr);
        ...
        len  -= p_flash_devinfo->dev_cfg.block_size;
        addr += p_flash_devinfo->dev_cfg.block_size;
    }
                         ...
                         void                *p_buf)
{
    ...
    /* 读取数据 */
    ret = awbl_flexspi_nor_flash_read(addr,
                                      p_buf,
                                      len);
                          ...
                          const void           *p_buf)
{
    ...
    while (length) {
        ...
        ret = awbl_flexspi_nor_flash_program(addr,
                                             ptr,
                                             bytes);
        if (ret != AW_OK) {
            goto __exit;
                                      ...
                                      const void                      *p_info)
{
    ...
    /* MTD device */
    p_fmtd->mtd.size              = p_finfo->nblks * p_flash_devinfo->dev_cfg.block_size;
    p_fmtd->mtd.type              = AW_MTD_TYPE_NOR_FLASH;
    p_fmtd->mtd.flags             = AW_MTD_FLAGS_NOR_FLASH;
    p_fmtd->mtd.erase_size        = p_flash_devinfo->dev_cfg.block_size;
    p_fmtd->mtd.write_size        = p_flash_devinfo->dev_cfg.page_size;
    ...
    p_fmtd->erase_align_mask = (~0) % p_flash_devinfo->dev_cfg.block_size;
    p_fmtd->addr_offset      = p_finfo->start_blk * p_flash_devinfo->dev_cfg.block_size;
