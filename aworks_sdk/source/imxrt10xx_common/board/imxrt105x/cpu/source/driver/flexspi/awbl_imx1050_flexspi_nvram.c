                                           ...
                                           int               len)
{
    ...
    int      copylen = 0;
    aw_err_t ret     = AW_OK;
    uint32_t sector_size = p_devinfo->dev_cfg.sector_size;
    uint32_t sector_cnt  = p_devinfo->dev_cfg.flash_size / sector_size;
    ...
    for(i = 0; i < p_devinfo->nvram_info.seglst_count; i++) {
        ...
        /* 读取数据 */
        ret = awbl_flexspi_nor_flash_read(p_seg->seg_addr + offset,
                                          (uint8_t *)p_buf,
                                          copylen);
                                           ...
                                           int              len)
{
    ...
    uint32_t  page_size   = p_devinfo->dev_cfg.page_size;
    uint32_t  sector_size = p_devinfo->dev_cfg.sector_size;
    uint32_t  sector_cnt  = p_devinfo->dev_cfg.flash_size / sector_size;
    ...
    for (i = 0; i < p_devinfo->nvram_info.seglst_count; i++) {
        if ((p_seg->unit == unit) && (strcmp(p_seg->p_name, p_name) == 0)) {
            ...
            for (j = 0; j < copylen;) {
                ...
                /* 读取一扇区数据 */
                ret = awbl_flexspi_nor_flash_read(sector_addr_start,
                                                  (uint8_t *)(p_devinfo->nvram_info.p_blk_buf),
                                                  sector_size);
                ...
                /* 擦除扇区 */
                awbl_flexspi_nor_flash_erase(sector_addr_start);
                ...
                for (n = 0; n < sector_page_cnt; n++) {
                    ...
                    ret = awbl_flexspi_nor_flash_program(sector_addr_start + n * page_size,
                    (uint8_t *)(p_devinfo->nvram_info.p_blk_buf + n * page_size),
                    page_size);
