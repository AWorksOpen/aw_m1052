...
/**
 * \file
 * \brief AW imx10xx eDMA 接口头文件
 *
 * 使用本模块需要包含以下头文件：
 * \code
 * #include "aw_imx10xx_edma.h"
 * \endcode
 *
 * 更多内容待添加。。。
 *
...
#ifndef __AW_IMX10xx_EDMA_H
#define __AW_IMX10xx_EDMA_H
...
/** \brief DMA 驱动名 */
#define AWBL_IMX10XX_EDMA_NAME                  "imx10xx_edma"
...
enum dma_chan_cfg_status {
    CHAN_CFG_0 = 0,  /* 未配置状态  */
    CHAN_CFG_1 = 1,  /* 调用imx10xx_edma_control(,DMA_SLAVE_CONFIG,,)进行了基础的配置 */
...
/**
 * \brief initialize AWBus imx10xx sdma service
 *
...
int imx10xx_edma_prep_memcpy(void *chan,
        dma_addr_t dma_dst,
        dma_addr_t dma_src,
        size_t len,
        unsigned long flags);

int imx10xx_edma_prep_sg(void *chan,
 ...
 *
 */
int imx10xx_edma_prep_memcpy_sg(void *chan,
        struct scatterlist *dst_sg,
        unsigned int dst_count,
        struct scatterlist *src_sg,
        unsigned int src_count,
...
int imx10xx_edma_prep_slave_sg(void *chan,
        struct scatterlist *src_sg,
        unsigned int src_count,
        enum dma_transfer_direction direction);
...
int imx10xx_edma_chan_start_async(void *chan,
