...
/** \brief get awbl_imx10xx_edma_chan_dev from awbl_dev */
#define __TO_GET_EDMA_DEV(p_dev)   \
       (awbl_imx10xx_edma_dev_t *)(p_dev)

/** \brief get awbl_imx10xx_edma_devinfo_t from awbl_dev */
#define __TO_GET_EDMA_DEVINFO(p_dev)   \
       (awbl_imx10xx_edma_devinfo_t *)AWBL_DEVINFO_GET(p_dev)
...
{
    uint32_t                     i;
    awbl_imx10xx_edma_dev_t     *edma           = __TO_GET_EDMA_DEV(p_dev);
    awbl_imx10xx_edma_devinfo_t *p_edma_devinfo = __TO_GET_EDMA_DEVINFO(p_dev);
    rt105x_edma_regs_t          *p_hw_edma      = (rt105x_edma_regs_t *)p_edma_devinfo->regbase;
    rt105x_dmamux_regs_t        *p_hw_dmamux    = (rt105x_dmamux_regs_t *)p_edma_devinfo->dmamux_regbase;
...
aw_local int __edma_channel_cfg (void *chan, struct dma_slave_config *cfg)
{
    ...
    awbl_imx10xx_edma_dev_t     *edma           = edmac->edma;
    awbl_imx10xx_edma_devinfo_t *p_edma_devinfo = __TO_GET_EDMA_DEVINFO(edma);
    rt105x_edma_regs_t          *p_hw_edma      = (rt105x_edma_regs_t *)p_edma_devinfo->regbase;
...
static void edma_intr_task_fn(void *p)
{
    uint32_t                     i;
    uint32_t                    ret;
    awbl_imx10xx_edma_dev_t     *edma = (awbl_imx10xx_edma_dev_t *)p;
    awbl_imx10xx_edma_devinfo_t *p_edma_devinfo = __TO_GET_EDMA_DEVINFO(p);
    rt105x_edma_regs_t          *p_hw_edma      = (rt105x_edma_regs_t *)p_edma_devinfo->regbase;
...
/**
 * \brief ISR for imx10xx eDMA interrupts
 *
 * This routine handles the imx10xx eDMA interrupts. This function
 * is hooked as the ISR in the second level initialization routine.
 */
aw_local void __imx10xx_edma_isr (void *p)
{
    uint32_t i;
    awbl_imx10xx_edma_dev_t     *edma = (awbl_imx10xx_edma_dev_t *)p;
    awbl_imx10xx_edma_devinfo_t *p_edma_devinfo = __TO_GET_EDMA_DEVINFO(p);
...
/* 错误中断处理函数 */
aw_local void __imx10xx_edma_err_isr (void *p)
{
    uint32_t                    i;
    uint32_t                    es, err;
    uint32_t                    count;
 ...
 * \brief 第二阶段初始化
 */
aw_local void __imx10xx_edma_inst_init2 (awbl_dev_t *p_dev)
{
    ...
    /* 使能中断 */
    for(i = 0; i < p_edma_devinfo->inum_len; i++) {
        aw_int_connect(p_edma_devinfo->inum[i], __imx10xx_edma_isr, edma);
        aw_int_enable(p_edma_devinfo->inum[i]);
    }

    /* 使能错误中断   */
    p_hw_edma->SEEI = (0x01 << 6);
    aw_int_connect(p_edma_devinfo->err_inum, __imx10xx_edma_err_isr, edma);
...
aw_local aw_const struct awbl_drvfuncs __g_edma_drvfuncs = {
    ...
    __imx10xx_edma_inst_init2,  /**< \brief 2ed inst init */
    NULL                        /**< \brief inst int connect */
};


aw_local const imx10xx_edma_serv_func_t __g_edma_serv_func = {
 ...
 */
aw_local aw_err_t __imx10xx_edma_serv_get (
        struct awbl_dev *p_dev, void *p_arg)
{
     awbl_imx10xx_edma_dev_t            *edma = __TO_GET_EDMA_DEV(p_dev);
     struct awbl_imx10xx_edma_service   *p;

     p = &edma->edma_serv;
     p->p_dev = p_dev;
     p->p_serv_func = &__g_edma_serv_func;

     *(struct awbl_imx10xx_edma_service **)p_arg = p;
    return AW_OK;
}


/* driver methods (must defined as aw_const) */
aw_local aw_const struct awbl_dev_method __g_imx10xx_edma_dev_methods[] = {
    AWBL_METHOD(awbl_imx10xx_edma_serv_func_get, __imx10xx_edma_serv_get),
    AWBL_METHOD_END
};
...
aw_local aw_const awbl_plb_drvinfo_t __g_edma_drv_registration = {
    {
        ...
        AWBL_IMX10XX_EDMA_NAME,                 /* p_drvname */
        &__g_edma_drvfuncs,                     /* p_busfuncs */
        __g_imx10xx_edma_dev_methods,           /* p_methods */
