...
static const struct awbl_imx10xx_edma_service     *__g_p_edma_serv = NULL;
...
/**
 * \brief allocate imx10xx edma service from instance (helper)
 *
 ...
 */
aw_local aw_err_t __imx10xx_edma_serv_alloc_helper (
    ...
    void            *p_arg)
{
    ...
    if (strcmp(AWBL_IMX10XX_EDMA_NAME,p_dev->p_driver->p_drvname) == 0) {
        awbl_method_handler_t   pfn_imx10xx_edma_get_serv = NULL;
        pfn_imx10xx_edma_get_serv = awbl_dev_method_get(
            p_dev, AWBL_METHOD_CALL(awbl_imx10xx_edma_serv_func_get));
        if (NULL != pfn_imx10xx_edma_get_serv) {
            pfn_imx10xx_edma_get_serv(p_dev, &__g_p_edma_serv);
        }
    }
    return AW_OK;
}
 ...
 */
static int __imx10xx_edma_serv_alloc (void)
{
    /* todo: count the number of service */

    awbl_dev_iterate(
        __imx10xx_edma_serv_alloc_helper, NULL, AWBL_ITERATE_INSTANCES);

    return AW_OK;
}

void *imx10xx_edma_alloc_chan (struct imx_dma_data *data)
{
    void    *p = NULL;
...
int imx10xx_edma_control (void *chan, enum dma_ctrl_cmd cmd, unsigned long arg)
{
    int ret = -AW_ENODEV;
...
int imx10xx_edma_prep_memcpy_sg(
    void                *chan,
    struct scatterlist *dst_sg,
    unsigned int        dst_count,
    struct scatterlist *src_sg,
...
int imx10xx_edma_prep_slave_sg(
    void                       *chan,
    struct scatterlist         *src_sg,
    unsigned int                src_count,
    enum dma_transfer_direction direction)
...
int imx10xx_edma_prep_dma_cyclic(void *chan,
...
int imx10xx_edma_chan_start_sync(void *chan, uint32_t timeout)
{
    int ret = -AW_ENODEV;
...
int imx10xx_edma_chan_start_async(
    void               *chan,
...
int imx10xx_edma_get_transfer_count(void *chan)
{
    int ret = -AW_ENODEV;
...
int imx10xx_edma_chan_stop(void *chan)
{
    int ret = -AW_ENODEV;
...
int imx10xx_edma_free_chan(void *chan)
{
    int ret = -AW_ENODEV;
 ...
 * \brief edma initial
 */
void awbl_imx10xx_edma_init (void)
{
    /* allocate edma services from instance tree */
    __imx10xx_edma_serv_alloc();
}

/** \brief AWBus method, used to get a AWBus imx10xx eDMA service
 *
 *    a driver can implement the method 'awbl_imx10xx_edma_serv_func_get'
 ...
 * \endcode
 */
AWBL_METHOD_DEF(awbl_imx10xx_edma_serv_func_get, "awbl_imx10xx_edma_serv_func_get");
