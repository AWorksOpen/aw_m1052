                 ...
                 uint32_t ui32Len)
{
  ...
  for (ui32Loop = 0; pcDecoded[ui32Loop] && (ui32Count < (ui32Len - 1)); ui32Loop++) {
    ...
    case '\'': {
      ui32Count += snprintf(&pcEncoded[ui32Count], (ui32Len - ui32Count),
                            "&#39;");
      break;
    }
    }
...
GetCGIParam(const char *pcName, char *pcParams[], char *pcValue[],
            int32_t iNumParams, aw_bool_t *pbError)
{
  int32_t i32Param;
  int32_t i32Value;
  aw_bool_t bRetcode;
...
uint32_t
GetCGIParamIpaddr(const char *pcName, char *pcParams[], char *pcValue[],
                  int32_t iNumParams, aw_bool_t *pbError)
{
  ip_addr_t addr;
  int32_t i32Param;
  ...
  if (i32Param != -1) {
    ...
    // We found the parameter so now get its value.
    //
    bRetcode = ipaddr_aton(pcValue[i32Param], &addr);

    if (bRetcode) {
      //
      // All is well - return the parameter value.
      //
      return addr.u_addr.ip4.addr;
    }
  }
