...
aw_bool_t IsValidHexDigit(const char cDigit);
unsigned char HexDigit(const char cDigit);
aw_bool_t DecodeHexEscape(const char *pcEncoded, char *pcDecoded);
...
uint32_t DecodeFormString(const  char *pcEncoded, char *pcDecoded,
                               uint32_t ui32Len);
aw_bool_t CheckDecimalParam(const char *pcValue, int32_t *pi32Value);
int32_t GetCGIParam(const char *pcName, char *pcParams[], char *pcValue[],
                 int32_t i32NumParams, aw_bool_t *pbError);
uint32_t GetCGIParamIpaddr(const char *pcName, char *pcParams[], char *pcValue[],
                 int32_t i32NumParams, aw_bool_t *pbError);
