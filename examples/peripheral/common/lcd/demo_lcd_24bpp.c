 ...
 *   3. 在LCD配置文件中配置相关默认参数，如__HW800480_BUS_BPP 24，__HW800480_WORD_LBPP 24，
 *      则表示24位像素数据在24线数据总线的宽度的LCD显示
 *   4. 如果LCD缓冲区是静态区段,则需要注意LCD该段的大小，不能低于一帧数据大小，
 *      如24bpp,800*480分辨率，一帧数据大小为800*480*3=1152000
 *
 * - 实验现象：
 *   1. 先进行屏幕校准；
 ...
 *      键入"LCD_BPP 24 24"命令，会配置LCD位像素深度（一个像素的大小）和 数据总线的宽度
 ...
 *   9. 该例程支持24位像素数据在16线数据总线的宽度的LCD显示，只需键入"LCD_BPP  24 16"命令和"LCD_RESET"命令即可
 *
 *
 * \par 源代码
 * \snippet demo_lcd_24bpp.c src_lcd_24bpp
 *
 * \internal
...
aw_local uint8_t  *__gp_frame_buffer;   /* frame buffer地址 */
...
{
    pixel_24bpp_t* p = (pixel_24bpp_t*)__gp_frame_buffer+(y*__g_x_res+x);
...
{
    int i, ylen;
    uint8_t color[][3] = {{0xff, 0x00, 0x00},   /* 红色 */
                          {0x00, 0xff, 0x00},   /* 绿色 */
                          {0x00, 0x00, 0xff},   /* 蓝色 */
                          {0xff, 0xff, 0x00},   /* 黄色 */
                          {0xff, 0x00, 0xff},   /* 紫色 */
...
aw_local const aw_shell_cmd_item_t __g_my_lcd_cmds[] = {
    ...
    {__shell_lcd_bpp,    "LCD_BPP",     "<word> <bus>,lcd bit pixel depth"},   /* 位像素深度 */
    ...
    while (1) {
        ...
        if (aw_ts_exec(sys_ts, &ts, 1) > 0) {
            if (ts.x < __g_x_res && ts.y < __g_y_res) {
                ...
                __set_pen_color(0xff, 0xff, 0xff); /* 设置画笔颜色 */
...
/******************************************************************************/
void demo_lcd_24bpp_entry (void)
{
    aw_task_id_t    tsk;
