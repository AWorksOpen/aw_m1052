 ...
 *   2. 需要在 aw_prj_params.h 里使能 AW_COM_NETWORK
 *
 * - 实验现象：
 *   01. 执行 "scan" 可以查看附近的可见热点
 *   02. 执行 "scan passive" 可以查看附近的所有热点
 *   03. 执行 "join AWWIFI meiyoumima" 加入名称为 AWWIFI, 密码为 meiyoumima 的热点
 ...
 *
 * - 备注：
 *   1.驱动中暂时只支持open，wpa2_aes，wpa2三种加密模式。
 *   2.如果wifi进入错误状态不能恢复，请按照以下步骤操作，进行软件复位：
 *     在shell界面输入：wifi，进入系统注册的wifi子命令集；
 *     在shell界面输入：pwr 0， 关闭wifi模块；
 *     在shell界面输入：pwr 1，开启wifi模块；
 *     在shell界面输入：CTRL+C，退出子命令集。
 *
 * \par 源代码
...
#define WIFI_DEV_NAME                       DE_WIFI_DEV_NAME
                                            ...
                                            aw_bool_t           complete)
{
    ...
    if (complete == AW_TRUE) {
        ...
        shell_printl(
            "%s",
            dhcps_get_host_name_by_macaddr(ap_dhcp_server(),
                                           (aw_eth_mac_t *)&sta->bssid));
    }
    return AW_OK;
}
...
void demo_wifi_entry (void)
{
...
#ifdef AW_VDEBUG
    {
        ...
        if (p_netif) {
            ...
            aw_netif_hwaddr_get(p_netif, mac.octet, 6);
            aw_netif_dev_close(p_netif);
            snprintf(old_hostname,
                     sizeof(old_hostname),
                     "%s",
                     aw_net_hostname_get());
            snprintf(new_hostname,
                     sizeof(new_hostname),
                     "%s-%02X%02X%02X",
                     old_hostname,
                     mac.octet[3],
            ...
            if(ret == AW_OK){
                aw_kprintf("make wifi as default network card");
            }
