...
#ifndef __AWBL_HWCONF_LCD_800600W080TR_H
#define __AWBL_HWCONF_LCD_800600W080TR_H

#ifdef AW_DEV_LCD_800600W080TR
/*******************************************************************************
  HW800600 配置信息
*******************************************************************************/
#include "driver/display/awbl_imx1050_tft_panel.h"

#define __HW800600_X_RES        800
#define __HW800600_Y_RES        600
#define __HW800600_BUS_BPP      16
#define __HW800600_WORD_LBPP    16

#define __HW800600_DCLK_F       39000000//24000000

#define DOTCLK_H_ACTIVE         __HW800600_X_RES

#define DOTCLK_H_PULSE_WIDTH    1
#define DOTCLK_HF_PORCH         400//210
#define DOTCLK_HB_PORCH         46

#define DOTCLK_V_ACTIVE         __HW800600_Y_RES

#define DOTCLK_V_PULSE_WIDTH    3
#define DOTCLK_VF_PORCH         8
#define DOTCLK_VB_PORCH         23


typedef struct __hw800600f_gpio_cfg {
    int         gpio;
    uint32_t    func;

} __hw800600f_gpio_cfg_t;


aw_local aw_const  __hw800600f_gpio_cfg_t __g_hw800600f_gpios_list[] = {
...
aw_local void __hw800600_plfm_init (void)
{
    int i = 0;
    int lcd_gpios[AW_NELEMENTS(__g_hw800600f_gpios_list)];

    for (i = 0; i < AW_NELEMENTS(__g_hw800600f_gpios_list); i++) {
        lcd_gpios[i] = __g_hw800600f_gpios_list[i].gpio;
    }
    ...
    if (aw_gpio_pin_request("lcd_gpios",
        ...
        for (i = 0; i < AW_NELEMENTS(__g_hw800600f_gpios_list); i++) {
            /* 配置功能引脚 */
            aw_gpio_pin_cfg(__g_hw800600f_gpios_list[i].gpio,
                            __g_hw800600f_gpios_list[i].func);
        }
    }
}

/* 设备信息 */
aw_local aw_const awbl_imx1050_tft_panel_devinfo_t  __g_hw800600f_param = {
    /* panel_info */
    {
        __HW800600_X_RES,               /* x_res */
        __HW800600_Y_RES,               /* y_res */
        __HW800600_BUS_BPP,             /* rgb硬件总线数 */
        __HW800600_WORD_LBPP,           /* 一个像素的长度，位为单位,如rgb565数据，该值为16 */
        __HW800600_DCLK_F,              /* dclk_f */
    ...
    /* plfm_init */
    __hw800600_plfm_init
};

/* 设备实例内存静态分配 */
aw_local awbl_imx1050_tft_panel_t __g_hw800600f;

#define AWBL_HWCONF_LCD_800600W080TR    \
    {                                   \
...
#endif  /* AWBL_HWCONF_LCD_800600W080TR */

#endif  /* __AWBL_HWCONF_LCD_800600W080TR */
