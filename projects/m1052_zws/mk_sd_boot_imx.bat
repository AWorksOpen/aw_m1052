...
set copy_dst_dir=%burn_tools_common_dir%\SD卡启动\M105x

echo.
echo 生成dcd文件...
%dcd_and_imx_dirs%\mk_dcd_file.exe debug_config\dcd.cfg debug_config\test.dcd
echo.
Rem echo 请选择一个需要生成sd启动文件的构建类型1)Debug 2)Release 3)MinSizeRel:
choice /c 123 /m "请选择一个需要生成sd启动文件的构建类型1)Debug 2)Release 3)MinSizeRel:"
set ret=%errorlevel%
...
echo 生成imx文件...
%dcd_and_imx_dirs%\mk_sd_boot_imx.exe %TYPE%\%project%.srec debug_config\test.dcd %TYPE%\%project%.imx
