...
/**
 * @file zlg_iot_mqtt_client.h
 * @brief Client definition for MQTT
 */

#ifndef ZLG_IOT_SDK_SRC_IOT_MQTT_CLIENT_H
#define ZLG_IOT_SDK_SRC_IOT_MQTT_CLIENT_H
...
typedef struct _Client ZLG_IoT_Client;
 ...
 *
 */
typedef void (*iot_disconnect_handler)(ZLG_IoT_Client *, void *);


/**
 * @brief Reconnecting Callback Handler Type
 *
 * Defining a TYPE for definition of Reconnecting callback function pointers.
 *
 */
typedef void (*iot_reconnecting_handler)(ZLG_IoT_Client *, void *);

/**
 * @brief Reconnected Callback Handler Type
 *
 * Defining a TYPE for definition of Reconnected callback function pointers.
 *
 */
typedef void (*iot_reconnected_handler)(ZLG_IoT_Client *, void *);
...
typedef struct {
  ...
  iot_reconnecting_handler reconnectingHandler;	///< Callback to be invoked upon reconnecting
  void *reconnectingHandlerData;			///< Data to pass as argument when reconnecting handler is called
  iot_reconnected_handler reconnectedHandler; ///< Callback to be invoked after reconnected
  void *reconnectedHandlerData; ///< Data to pass as argument when reconnected handler is called
#ifdef _ENABLE_THREAD_SUPPORT_
...
#endif
  char *interfaceName;               ///< Bind this socket to a particular device like “eth0”, as specified in the passed interface name.
} IoT_Client_Init_Params;
extern const IoT_Client_Init_Params iotClientInitParamsDefault;

#ifdef _ENABLE_THREAD_SUPPORT_
#define IoT_Client_Init_Params_initializer { true, NULL, 0, NULL, NULL, NULL, 2000, 20000, 5000, true, NULL, NULL, NULL, NULL, NULL, NULL, false, NULL }
#else
#define IoT_Client_Init_Params_initializer { true, NULL, 0, NULL, NULL, NULL, 2000, 20000, 5000, true, NULL, NULL, NULL, NULL, NULL, NULL, NULL }
#endif
 ...
 *
 */
typedef void (*pApplicationHandler_t)(ZLG_IoT_Client *pClient, char *pTopicName, uint16_t topicNameLen,
...
typedef struct _ClientData {
	...
	unsigned char writeBuf[ZLG_IOT_MQTT_TX_BUF_LEN];
	unsigned char readBuf[ZLG_IOT_MQTT_RX_BUF_LEN];
	...
	MessageHandlers messageHandlers[ZLG_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS];
	iot_disconnect_handler disconnectHandler;

	void *disconnectHandlerData;

  iot_reconnecting_handler reconnectingHandler;
  void *reconnectingHandlerData;

  iot_reconnected_handler reconnectedHandler;
  void *reconnectedHandlerData;
} ClientData;
...
/**
 * @brief Set the IoT Client reconnecting handler
 *
 * Called to set the IoT Client reconnecting handler
 * The reconnecting handler is called whenever the client reconnecting
 *
 * @param pClient Reference to the IoT Client
 * @param pReconnectingHandler Reference to the new Reconnect Handler
 * @param pReconnectingHandlerData Reference to the data to be passed as argument when reconnect handler is called
 *
 * @return IoT_Error_t Type defining successful/failed API call
 */
...
/**
 * @brief Set the IoT Client reconnected handler
 *
 * Called to set the IoT Client reconnect handler
 * The reconnect handler is called whenever the client reconnect successful
 *
 * @param pClient Reference to the IoT Client
 * @param pReconnectedHandler Reference to the new Reconnect Handler
 * @param pReconnectedHandlerData Reference to the data to be passed as argument when reconnect handler is called
 *
 * @return IoT_Error_t Type defining successful/failed API call
 */
...
/**
 * @brief set default message handler
 */
