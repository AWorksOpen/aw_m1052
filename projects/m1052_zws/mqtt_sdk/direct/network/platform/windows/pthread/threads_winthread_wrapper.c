...
IoT_Error_t aws_iot_thread_mutex_init(IoT_Mutex_t *pMutex) {
  pMutex->lock = CreateMutex(NULL, false, NULL);
  if(NULL != pMutex->lock) {
...
IoT_Error_t aws_iot_thread_mutex_lock(IoT_Mutex_t *pMutex) {
  DWORD rc = WaitForSingleObject(pMutex->lock, INFINITE);
  if(WAIT_OBJECT_0 != rc) {
...
IoT_Error_t aws_iot_thread_mutex_trylock(IoT_Mutex_t *pMutex) {
  DWORD rc = WaitForSingleObject(pMutex->lock, 1000);
  if(WAIT_OBJECT_0 != rc) {
    return MUTEX_LOCK_ERROR;
  }
