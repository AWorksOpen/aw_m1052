...
/**
 * @file zlg_iot_mqtt_client_common_internal.c
 ...
 */
size_t zlg_iot_mqtt_internal_write_len_to_buffer(unsigned char *buf, uint32_t length) {
	size_t outLen = 0;
 ...
 */
IoT_Error_t zlg_iot_mqtt_internal_decode_remaining_length_from_buffer(unsigned char *buf, uint32_t *decodedLen,
...
uint32_t zlg_iot_mqtt_internal_get_final_packet_length_from_remaining_length(uint32_t rem_len) {
 ...
 */
uint16_t zlg_iot_mqtt_internal_read_uint16_t(unsigned char **pptr) {
 ...
 */
void zlg_iot_mqtt_internal_write_uint_16(unsigned char **pptr, uint16_t anInt) {
 ...
 */
unsigned char zlg_iot_mqtt_internal_read_char(unsigned char **pptr) {
 ...
 */
void zlg_iot_mqtt_internal_write_char(unsigned char **pptr, unsigned char c) {
	**pptr = c;
	(*pptr)++;
}

void zlg_iot_mqtt_internal_write_utf8_string(unsigned char **pptr, const char *string, uint16_t stringLen) {
	/* Nothing that calls this function will have a stringLen with a size larger than 2 bytes (MQTT 3.1.1 - 1.5.3) */
	zlg_iot_mqtt_internal_write_uint_16(pptr, stringLen);
	if(stringLen > 0) {
 ...
 */
IoT_Error_t zlg_iot_mqtt_internal_init_header(MQTTHeader *pHeader, MessageTypes message_type,
...
IoT_Error_t zlg_iot_mqtt_internal_send_packet(ZLG_IoT_Client *pClient, size_t length, Timer *pTimer) {
	...
	rc = zlg_iot_mqtt_client_lock_mutex(pClient, &(pClient->clientData.tls_write_mutex));
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
#endif
...
#ifdef _ENABLE_THREAD_SUPPORT_
	rc = zlg_iot_mqtt_client_unlock_mutex(pClient, &(pClient->clientData.tls_write_mutex));
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
#endif
...
static IoT_Error_t _zlg_iot_mqtt_internal_decode_packet_remaining_len(ZLG_IoT_Client *pClient,
...
static IoT_Error_t _zlg_iot_mqtt_internal_read_packet(ZLG_IoT_Client *pClient, Timer *pTimer, uint8_t *pPacketType) {
  ...
  memset(pClient->clientData.readBuf, 0x00, pClient->clientData.readBufSize);
	...
	rc = _zlg_iot_mqtt_internal_decode_packet_remaining_len(pClient, &rem_len, pTimer);
	if(SUCCESS != rc) {
		return rc;
	}
	...
	len += zlg_iot_mqtt_internal_write_len_to_buffer(pClient->clientData.readBuf + 1, (uint32_t) rem_len);
...
static bool _zlg_iot_mqtt_internal_is_topic_matched(char *pTopicFilter, char *pTopicName, uint16_t topicNameLen) {
...
static void* s_default_message_handler_ctx = NULL;
static pApplicationHandler_t s_default_message_handler = NULL;

void zlg_iot_mqtt_set_default_message_handler(pApplicationHandler_t message_handler, void* ctx) {
  s_default_message_handler = message_handler;
  s_default_message_handler_ctx = ctx;
}

static void zlg_iot_mqtt_deliver_to_default_message_handler(ZLG_IoT_Client *pClient, char *pTopicName,
    uint16_t topicNameLen, IoT_Publish_Message_Params *pMessageParams) {
  if(s_default_message_handler) {
    s_default_message_handler(pClient, pTopicName, topicNameLen, pMessageParams, s_default_message_handler_ctx);
  }
}

static IoT_Error_t _zlg_iot_mqtt_internal_deliver_message(ZLG_IoT_Client *pClient, char *pTopicName,
	...
	clientState = zlg_iot_mqtt_get_client_state(pClient);
	zlg_iot_mqtt_set_client_state(pClient, clientState, CLIENT_STATE_CONNECTED_WAIT_FOR_CB_RETURN);

	/* Find the right message handler - indexed by topic */
	for(itr = 0; itr < ZLG_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS; ++itr) {
		if(NULL != pClient->clientData.messageHandlers[itr].topicName) {
			if(((topicNameLen == pClient->clientData.messageHandlers[itr].topicNameLen)
			   ...
			   || _zlg_iot_mqtt_internal_is_topic_matched((char *) pClient->clientData.messageHandlers[itr].topicName,
	...
	zlg_iot_mqtt_deliver_to_default_message_handler(pClient, pTopicName, topicNameLen, pMessageParams);

	rc = zlg_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTED_WAIT_FOR_CB_RETURN, clientState);

	FUNC_EXIT_RC(rc);
}

static IoT_Error_t _zlg_iot_mqtt_internal_handle_publish(ZLG_IoT_Client *pClient, Timer *pTimer) {
	char *topicName;
	uint16_t topicNameLen;
	uint32_t len;
	IoT_Error_t rc;
	...
	rc = zlg_iot_mqtt_internal_deserialize_publish(&msg.isDup, &msg.qos, &msg.isRetained,
	...
	if(QOS0 != msg.qos) {
          ...
          rc = zlg_iot_mqtt_internal_serialize_ack(pClient->clientData.writeBuf, pClient->clientData.writeBufSize,
                                                                                         PUBACK, 0, msg.id, &len);
          ...
          rc = zlg_iot_mqtt_internal_send_packet(pClient, len, pTimer);
          if(SUCCESS != rc) {
                  FUNC_EXIT_RC(rc);
          }
	}

        rc = _zlg_iot_mqtt_internal_deliver_message(pClient, topicName, topicNameLen, &msg);
        if(SUCCESS != rc) {
                FUNC_EXIT_RC(rc);
        }
...
IoT_Error_t zlg_iot_mqtt_internal_cycle_read(ZLG_IoT_Client *pClient, Timer *pTimer, uint8_t *pPacketType) {
	IoT_Error_t rc;
...
#ifdef _ENABLE_THREAD_SUPPORT_
	threadRc = zlg_iot_mqtt_client_lock_mutex(pClient, &(pClient->clientData.tls_read_mutex));
	if(SUCCESS != threadRc) {
	...
	rc = _zlg_iot_mqtt_internal_read_packet(pClient, pTimer, pPacketType);

#ifdef _ENABLE_THREAD_SUPPORT_
	threadRc = zlg_iot_mqtt_client_unlock_mutex(pClient, &(pClient->clientData.tls_read_mutex));
	...
	switch(*pPacketType) {
		...
		case UNSUBACK:
			...
			break;
		case PUBLISH: {
			/*use packetTimer instead of pTimer，ensure PUBACK can be send out*/
			Timer packetTimer;
			init_timer(&packetTimer);
			countdown_ms(&packetTimer, pClient->clientData.packetTimeoutMs);
			rc = _zlg_iot_mqtt_internal_handle_publish(pClient, &packetTimer);
			break;
		}
		case PUBREC:
		case PUBCOMP:
...
IoT_Error_t zlg_iot_mqtt_internal_wait_for_read(ZLG_IoT_Client *pClient, uint8_t packetType, Timer *pTimer) {
	IoT_Error_t rc;
	...
	do {
		...
		rc = zlg_iot_mqtt_internal_cycle_read(pClient, pTimer, &read_packet_type);
...
/**
  ...
  */
IoT_Error_t zlg_iot_mqtt_internal_serialize_zero(unsigned char *pTxBuf, size_t txBufLen, MessageTypes packetType,
	...
	rc = zlg_iot_mqtt_internal_init_header(&header, packetType, QOS0, 0, 0);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	/* write header */
	zlg_iot_mqtt_internal_write_char(&ptr, header.byte);

	/* write remaining length */
	ptr += zlg_iot_mqtt_internal_write_len_to_buffer(ptr, 0);
