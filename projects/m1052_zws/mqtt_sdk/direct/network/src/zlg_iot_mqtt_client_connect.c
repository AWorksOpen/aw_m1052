...
/**
 * @file zlg_iot_mqtt_client_connect.c
...
/**
  ...
  */
static uint32_t _zlg_iot_get_connect_packet_length(IoT_Client_Connect_Params *pConnectParams) {
	uint32_t len;
...
/**
  ...
  * @return IoT_Error_t indicating function execution status
  */
static IoT_Error_t _zlg_iot_mqtt_serialize_connect(unsigned char *pTxBuf, size_t txBufLen,
	...
	ptr = pTxBuf;
	len = _zlg_iot_get_connect_packet_length(pConnectParams);
	if(zlg_iot_mqtt_internal_get_final_packet_length_from_remaining_length(len) > txBufLen) {
		FUNC_EXIT_RC(MQTT_TX_BUFFER_TOO_SHORT_ERROR);
	}

	rc = zlg_iot_mqtt_internal_init_header(&header, CONNECT, QOS0, 0, 0);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	zlg_iot_mqtt_internal_write_char(&ptr, header.byte); /* write header */

	ptr += zlg_iot_mqtt_internal_write_len_to_buffer(ptr, len); /* write remaining length */

	// Enable if adding support for more versions
	//if(MQTT_3_1_1 == pConnectParams->MQTTVersion) {
	zlg_iot_mqtt_internal_write_utf8_string(&ptr, "MQTT", 4);
	zlg_iot_mqtt_internal_write_char(&ptr, (unsigned char) pConnectParams->MQTTVersion);
	//}
	...
	zlg_iot_mqtt_internal_write_char(&ptr, flags.all);
	zlg_iot_mqtt_internal_write_uint_16(&ptr, pConnectParams->keepAliveIntervalInSec);
	...
	if(NULL == pConnectParams->pClientID) {
		zlg_iot_mqtt_internal_write_uint_16(&ptr, 0);
	} else {
		zlg_iot_mqtt_internal_write_utf8_string(&ptr, pConnectParams->pClientID, pConnectParams->clientIDLen);
	}

	if(pConnectParams->isWillMsgPresent) {
		zlg_iot_mqtt_internal_write_utf8_string(&ptr, pConnectParams->will.pTopicName,
												pConnectParams->will.topicNameLen);
		zlg_iot_mqtt_internal_write_utf8_string(&ptr, pConnectParams->will.pMessage, pConnectParams->will.msgLen);
	}

	if(pConnectParams->pUsername) {
		zlg_iot_mqtt_internal_write_utf8_string(&ptr, pConnectParams->pUsername, pConnectParams->usernameLen);
	}

	if(pConnectParams->pPassword) {
		zlg_iot_mqtt_internal_write_utf8_string(&ptr, pConnectParams->pPassword, pConnectParams->passwordLen);
	}
...
/**
  ...
  * @return IoT_Error_t indicating function execution status
  */
static IoT_Error_t _zlg_iot_mqtt_deserialize_connack(unsigned char *pSessionPresent, IoT_Error_t *pConnackRc,
	...
	header.byte = zlg_iot_mqtt_internal_read_char(&curdata);
	if(CONNACK != MQTT_HEADER_FIELD_TYPE(header.byte)) {
		FUNC_EXIT_RC(FAILURE);
	}

	/* read remaining length */
	rc = zlg_iot_mqtt_internal_decode_remaining_length_from_buffer(curdata, &decodedLen, &readBytesLen);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	...
	flags.all = zlg_iot_mqtt_internal_read_char(&curdata);
	*pSessionPresent = flags.bits.sessionpresent;
	connack_rc_char = zlg_iot_mqtt_internal_read_char(&curdata);
 ...
 */
static bool _zlg_iot_mqtt_is_client_state_valid_for_connect(ClientState clientState) {
	bool isValid = false;
 ...
 */
static IoT_Error_t _zlg_iot_mqtt_internal_connect(ZLG_IoT_Client *pClient, IoT_Client_Connect_Params *pConnectParams) {
	Timer connect_timer;
	...
	if(NULL != pConnectParams) {
		...
		rc = zlg_iot_mqtt_set_connect_params(pClient, pConnectParams);
		if(SUCCESS != rc) {
	...
	rc = _zlg_iot_mqtt_serialize_connect(pClient->clientData.writeBuf, pClient->clientData.writeBufSize,
										 &(pClient->clientData.options), &len);
	if(SUCCESS != rc || 0 >= len) {
		FUNC_EXIT_RC(rc);
		IOT_ERROR("Connect: connect failed.\n");
	}

	/* send the connect packet */
	rc = zlg_iot_mqtt_internal_send_packet(pClient, len, &connect_timer);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
		IOT_ERROR("Connect: send packet failed.\n");
	}

	/* this will be a blocking call, wait for the CONNACK */
	rc = zlg_iot_mqtt_internal_wait_for_read(pClient, CONNACK, &connect_timer);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
		IOT_ERROR("Connect: read packet failed.\n");
	}

	/* Received CONNACK, check the return code */
	rc = _zlg_iot_mqtt_deserialize_connack((unsigned char *) &sessionPresent, &connack_rc, pClient->clientData.readBuf,
										   pClient->clientData.readBufSize);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
		IOT_ERROR("Connect: decode packet failed.\n");
	}

	if(MQTT_CONNACK_CONNECTION_ACCEPTED != connack_rc) {
		FUNC_EXIT_RC(connack_rc);
		IOT_ERROR("Connect: not accepted.\n");
	}
 ...
 * @return An IoT Error Type defining successful/failed connection
 */
IoT_Error_t zlg_iot_mqtt_connect(ZLG_IoT_Client *pClient, IoT_Client_Connect_Params *pConnectParams) {
	...
	if(false == _zlg_iot_mqtt_is_client_state_valid_for_connect(clientState)) {
	...
	zlg_iot_mqtt_set_client_state(pClient, clientState, CLIENT_STATE_CONNECTING);

	rc = _zlg_iot_mqtt_internal_connect(pClient, pConnectParams);
	...
	if(SUCCESS != rc) {
		...
		zlg_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTING, CLIENT_STATE_DISCONNECTED_ERROR);
	} else {
		zlg_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTING, CLIENT_STATE_CONNECTED_IDLE);
	}
 ...
 */
IoT_Error_t _zlg_iot_mqtt_internal_disconnect(ZLG_IoT_Client *pClient) {
	...
	rc = zlg_iot_mqtt_internal_serialize_zero(pClient->clientData.writeBuf, pClient->clientData.writeBufSize,
											  DISCONNECT,
											  &serialized_len);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	...
	if(serialized_len > 0) {
		zlg_iot_mqtt_internal_send_packet(pClient, serialized_len, &timer);
	}
 ...
 * @return An IoT Error Type defining successful/failed send of the disconnect control packet.
 */
IoT_Error_t zlg_iot_mqtt_disconnect(ZLG_IoT_Client *pClient) {
	ClientState clientState;
	IoT_Error_t rc;
	...
	clientState = zlg_iot_mqtt_get_client_state(pClient);
	if(!zlg_iot_mqtt_is_client_connected(pClient)) {
		/* Network is already disconnected. Do nothing */
		FUNC_EXIT_RC(NETWORK_DISCONNECTED_ERROR);
	}

	rc = zlg_iot_mqtt_set_client_state(pClient, clientState, CLIENT_STATE_DISCONNECTING);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	rc = _zlg_iot_mqtt_internal_disconnect(pClient);
 ...
 * @return An IoT Error Type defining successful/failed connection
 */
IoT_Error_t zlg_iot_mqtt_attempt_reconnect(ZLG_IoT_Client *pClient) {
	IoT_Error_t rc;
	...
	rc = zlg_iot_mqtt_connect(pClient, NULL);

	/* If still disconnected handle disconnect */
	if(CLIENT_STATE_CONNECTED_IDLE != zlg_iot_mqtt_get_client_state(pClient)) {
		zlg_iot_mqtt_set_client_state(pClient, CLIENT_STATE_DISCONNECTED_ERROR, CLIENT_STATE_PENDING_RECONNECT);
		FUNC_EXIT_RC(NETWORK_ATTEMPTING_RECONNECT);
	}

	rc = zlg_iot_mqtt_resubscribe(pClient);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
