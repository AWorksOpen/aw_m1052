...
/**
 * @file zlg_iot_mqtt_client_publish.c
 ...
 */
static IoT_Error_t _zlg_iot_mqtt_read_string_with_len(char **stringVar, uint16_t *stringLen,
	...
	if(enddata - (*pptr) > 1) {
		*stringLen = zlg_iot_mqtt_internal_read_uint16_t(pptr); /* increments pptr to point past length */
...
/**
  ...
  */
static IoT_Error_t _zlg_iot_mqtt_internal_serialize_publish(unsigned char *pTxBuf, size_t txBufLen, uint8_t dup,
	...
	rc = zlg_iot_mqtt_internal_init_header(&header, PUBLISH, qos, dup, retained);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	zlg_iot_mqtt_internal_write_char(&ptr, header.byte); /* write header */

	ptr += zlg_iot_mqtt_internal_write_len_to_buffer(ptr, rem_len); /* write remaining length */;

	zlg_iot_mqtt_internal_write_utf8_string(&ptr, pTopicName, topicNameLen);
...
/**
  ...
  * @return An IoT Error Type defining successful/failed call
  */
IoT_Error_t zlg_iot_mqtt_internal_serialize_ack(unsigned char *pTxBuf, size_t txBufLen,
	...
	rc = zlg_iot_mqtt_internal_init_header(&header, msgType, requestQoS, dup, 0);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	zlg_iot_mqtt_internal_write_char(&ptr, header.byte); /* write header */

	ptr += zlg_iot_mqtt_internal_write_len_to_buffer(ptr, 2); /* write remaining length */
	zlg_iot_mqtt_internal_write_uint_16(&ptr, packetId);
	*pSerializedLen = (uint32_t) (ptr - pTxBuf);
 ...
 */
static IoT_Error_t _zlg_iot_mqtt_internal_publish(ZLG_IoT_Client *pClient, const char *pTopicName,
	...
	if(QOS1 == pParams->qos) {
		pParams->id = zlg_iot_mqtt_get_next_packet_id(pClient);
	}

	rc = _zlg_iot_mqtt_internal_serialize_publish(pClient->clientData.writeBuf, pClient->clientData.writeBufSize, 0,
	...
	if(QOS1 == pParams->qos) {
		rc = zlg_iot_mqtt_internal_wait_for_read(pClient, PUBACK, &timer);
		if(SUCCESS != rc) {
			FUNC_EXIT_RC(rc);
		}

		rc = zlg_iot_mqtt_internal_deserialize_ack(&type, &dup, &packet_id, pClient->clientData.readBuf,
 ...
 * @return An IoT Error Type defining successful/failed publish
 */
IoT_Error_t zlg_iot_mqtt_publish(ZLG_IoT_Client *pClient, const char *pTopicName, uint16_t topicNameLen,
	...
	rc = zlg_iot_mqtt_set_client_state(pClient, clientState, CLIENT_STATE_CONNECTED_PUBLISH_IN_PROGRESS);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	pubRc = _zlg_iot_mqtt_internal_publish(pClient, pTopicName, topicNameLen, pParams);

	rc = zlg_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTED_PUBLISH_IN_PROGRESS, clientState);
...
/**
  ...
  * @return An IoT Error Type defining successful/failed call
  */
IoT_Error_t zlg_iot_mqtt_internal_deserialize_publish(uint8_t *dup, QoS *qos,
	...
	*retained = MQTT_HEADER_FIELD_RETAIN(header.byte);
	...
	if(SUCCESS != _zlg_iot_mqtt_read_string_with_len(pTopicName, topicNameLen, &curData, endData)
	   || (0 > (endData - curData))) {
		FUNC_EXIT_RC(FAILURE);
	}
...
/**
  ...
  * @return An IoT Error Type defining successful/failed call
  */
IoT_Error_t zlg_iot_mqtt_internal_deserialize_ack(unsigned char *pPacketType, unsigned char *dup,
