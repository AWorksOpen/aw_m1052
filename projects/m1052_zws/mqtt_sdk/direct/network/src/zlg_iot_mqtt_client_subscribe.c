...
/**
 * @file zlg_iot_mqtt_client_subscribe.c
...
/**
  ...
  */
static IoT_Error_t _zlg_iot_mqtt_serialize_subscribe(unsigned char *pTxBuf, size_t txBufLen,
	...
	if(zlg_iot_mqtt_internal_get_final_packet_length_from_remaining_length(rem_len) > txBufLen) {
		FUNC_EXIT_RC(MQTT_TX_BUFFER_TOO_SHORT_ERROR);
	}

	rc = zlg_iot_mqtt_internal_init_header(&header, SUBSCRIBE, QOS1, dup, 0);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	/* write header */
	...
	ptr += zlg_iot_mqtt_internal_write_len_to_buffer(ptr, rem_len);

	zlg_iot_mqtt_internal_write_uint_16(&ptr, packetId);

	for(itr = 0; itr < topicCount; ++itr) {
		zlg_iot_mqtt_internal_write_utf8_string(&ptr, pTopicNameList[itr], pTopicNameLenList[itr]);
		zlg_iot_mqtt_internal_write_char(&ptr, (unsigned char) pRequestedQoSs[itr]);
	}
...
/**
  ...
  * @return An IoT Error Type defining successful/failed operation
  */
static IoT_Error_t _zlg_iot_mqtt_deserialize_suback(uint16_t *pPacketId, uint32_t maxExpectedQoSCount,
	...
	decodeRc = zlg_iot_mqtt_internal_decode_remaining_length_from_buffer(curData, &decodedLen, &readBytesLen);
	if(SUCCESS != decodeRc) {
	...
	*pPacketId = zlg_iot_mqtt_internal_read_uint16_t(&curData);
	...
	*pGrantedQoSCount = 0;
	while(curData < endData) {
		...
		pGrantedQoSs[(*pGrantedQoSCount)++] = (QoS) zlg_iot_mqtt_internal_read_char(&curData);
	}
...
static uint32_t _zlg_iot_mqtt_get_free_message_handler_index(ZLG_IoT_Client *pClient) {
	uint32_t itr;
 ...
 */
static IoT_Error_t _zlg_iot_mqtt_internal_subscribe(ZLG_IoT_Client *pClient, const char *pTopicName,
	...
	serializedLen = 0;
	count = 0;
	txPacketId = zlg_iot_mqtt_get_next_packet_id(pClient);
	rxPacketId = 0;

	rc = _zlg_iot_mqtt_serialize_subscribe(pClient->clientData.writeBuf, pClient->clientData.writeBufSize, 0,
	...
	rc = zlg_iot_mqtt_internal_send_packet(pClient, serializedLen, &timer);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	/* wait for suback */
	rc = zlg_iot_mqtt_internal_wait_for_read(pClient, SUBACK, &timer);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	/* Granted QoS can be 0, 1 or 2 */
	rc = _zlg_iot_mqtt_deserialize_suback(&rxPacketId, 1, &count, grantedQoS, pClient->clientData.readBuf,
  ...
  //Lixianjing: 对于网关来说，可能需要注册成千上万个主题，而使用同一个回调函数。
  //所以我们允许注册时不提供回调函数，而是提供一个全局的回调函数即可。
  if(pApplicationHandler) {
    indexOfFreeMessageHandler = _zlg_iot_mqtt_get_free_message_handler_index(pClient);
    if(ZLG_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS <= indexOfFreeMessageHandler) {
 ...
 * @return An IoT Error Type defining successful/failed subscription
 */
IoT_Error_t zlg_iot_mqtt_subscribe(ZLG_IoT_Client *pClient, const char *pTopicName, uint16_t topicNameLen,
	...
	if(NULL == pClient || NULL == pTopicName) {
		FUNC_EXIT_RC(NULL_VALUE_ERROR);
	}
	...
	rc = zlg_iot_mqtt_set_client_state(pClient, clientState, CLIENT_STATE_CONNECTED_SUBSCRIBE_IN_PROGRESS);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	subRc = _zlg_iot_mqtt_internal_subscribe(pClient, pTopicName, topicNameLen, qos,
											 pApplicationHandler, pApplicationHandlerData);

	rc = zlg_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTED_SUBSCRIBE_IN_PROGRESS, clientState);
 ...
 * @return An IoT Error Type defining successful/failed subscription
 */
static IoT_Error_t _zlg_iot_mqtt_internal_resubscribe(ZLG_IoT_Client *pClient) {
	uint16_t packetId;
	...
	len = 0;
	count = 0;
	existingSubCount = _zlg_iot_mqtt_get_free_message_handler_index(pClient);
	...
	for(itr = 0; itr < existingSubCount; itr++) {
		...
		rc = _zlg_iot_mqtt_serialize_subscribe(pClient->clientData.writeBuf, pClient->clientData.writeBufSize, 0,
											   zlg_iot_mqtt_get_next_packet_id(pClient), 1,
		...
		/* send the subscribe packet */
		rc = zlg_iot_mqtt_internal_send_packet(pClient, len, &timer);
		if(SUCCESS != rc) {
			FUNC_EXIT_RC(rc);
		}
		...
		/* Granted QoS can be 0, 1 or 2 */
		rc = _zlg_iot_mqtt_deserialize_suback(&packetId, 1, &count, grantedQoS, pClient->clientData.readBuf,
											  pClient->clientData.readBufSize);
		if(SUCCESS != rc) {
			FUNC_EXIT_RC(rc);
		}
 ...
 * @return An IoT Error Type defining successful/failed subscription
 */
IoT_Error_t zlg_iot_mqtt_resubscribe(ZLG_IoT_Client *pClient) {
	...
	if(false == zlg_iot_mqtt_is_client_connected(pClient)) {
		FUNC_EXIT_RC(NETWORK_DISCONNECTED_ERROR);
	}
	...
	rc = zlg_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTED_IDLE,
									   CLIENT_STATE_CONNECTED_RESUBSCRIBE_IN_PROGRESS);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	resubRc = _zlg_iot_mqtt_internal_resubscribe(pClient);

	rc = zlg_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTED_RESUBSCRIBE_IN_PROGRESS,
