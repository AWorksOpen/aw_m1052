...
/**
 * @file zlg_iot_mqtt_client_unsubscribe.c
...
/**
  ...
  */
static IoT_Error_t _zlg_iot_mqtt_serialize_unsubscribe(unsigned char *pTxBuf, size_t txBufLen,
	...
	rc = zlg_iot_mqtt_internal_init_header(&header, UNSUBSCRIBE, QOS1, dup, 0);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	...
	for(i = 0; i < count; ++i) {
		zlg_iot_mqtt_internal_write_utf8_string(&ptr, pTopicNameList[i], pTopicNameLenList[i]);
	}
...
/**
  ...
  * @return IoT_Error_t indicating function execution status
  */
static IoT_Error_t _zlg_iot_mqtt_deserialize_unsuback(uint16_t *pPacketId, unsigned char *pRxBuf, size_t rxBufLen) {
	...
	rc = zlg_iot_mqtt_internal_deserialize_ack(&type, &dup, pPacketId, pRxBuf, rxBufLen);
 ...
 */
static IoT_Error_t _zlg_iot_mqtt_internal_unsubscribe(ZLG_IoT_Client *pClient, const char *pTopicFilter,
	...
	rc = _zlg_iot_mqtt_serialize_unsubscribe(pClient->clientData.writeBuf, pClient->clientData.writeBufSize, 0,
											 zlg_iot_mqtt_get_next_packet_id(pClient), 1, &pTopicFilter,
	...
	rc = zlg_iot_mqtt_internal_wait_for_read(pClient, UNSUBACK, &timer);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}

	rc = _zlg_iot_mqtt_deserialize_unsuback(&packet_id, pClient->clientData.readBuf, pClient->clientData.readBufSize);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
 ...
 * @return An IoT Error Type defining successful/failed unsubscribe call
 */
IoT_Error_t zlg_iot_mqtt_unsubscribe(ZLG_IoT_Client *pClient, const char *pTopicFilter, uint16_t topicFilterLen) {
	...
	rc = zlg_iot_mqtt_set_client_state(pClient, clientState, CLIENT_STATE_CONNECTED_UNSUBSCRIBE_IN_PROGRESS);
	if(SUCCESS != rc) {
		rc = zlg_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTED_UNSUBSCRIBE_IN_PROGRESS, clientState);
		return rc;
	}

	unsubRc = _zlg_iot_mqtt_internal_unsubscribe(pClient, pTopicFilter, topicFilterLen);
