...
/**
 * @file zlg_iot_mqtt_client_yield.c
...
/**
  * This is for the case when the zlg_iot_mqtt_internal_send_packet Fails.
  */
static void _zlg_iot_mqtt_force_client_disconnect(ZLG_IoT_Client *pClient) {
...
static IoT_Error_t _zlg_iot_mqtt_handle_disconnect(ZLG_IoT_Client *pClient) {
	IoT_Error_t rc;

	FUNC_ENTRY;

	rc = zlg_iot_mqtt_disconnect(pClient);
	if(rc != SUCCESS) {
		// If the zlg_iot_mqtt_internal_send_packet prevents us from sending a disconnect packet then we have to clean the stack
		_zlg_iot_mqtt_force_client_disconnect(pClient);
	}
...
static IoT_Error_t _zlg_iot_mqtt_handle_reconnect(ZLG_IoT_Client *pClient) {
	IoT_Error_t rc;
    ...
    if(NULL != pClient->clientData.reconnectingHandler) {
      pClient->clientData.reconnectingHandler(pClient, pClient->clientData.reconnectingHandlerData);
    }

		rc = zlg_iot_mqtt_attempt_reconnect(pClient);
		if(NETWORK_RECONNECTED == rc) {
      if(NULL != pClient->clientData.reconnectedHandler) {
        pClient->clientData.reconnectedHandler(pClient, pClient->clientData.reconnectedHandlerData);
      }
	...
	if(ZLG_IOT_MQTT_MAX_RECONNECT_WAIT_INTERVAL < pClient->clientData.currentReconnectWaitInterval) {
...
static IoT_Error_t _zlg_iot_mqtt_keep_alive(ZLG_IoT_Client *pClient) {
	...
	if(pClient->clientStatus.isPingOutstanding) {
		rc = _zlg_iot_mqtt_handle_disconnect(pClient);
		FUNC_EXIT_RC(rc);
	}
	...
	rc = zlg_iot_mqtt_internal_send_packet(pClient, serialized_len, &timer);
	if(SUCCESS != rc) {
 ...
 */
static IoT_Error_t _zlg_iot_mqtt_internal_yield(ZLG_IoT_Client *pClient, uint32_t timeout_ms) {
	...
	do {
		...
		if(CLIENT_STATE_PENDING_RECONNECT == clientState) {
			...
			yieldRc = _zlg_iot_mqtt_handle_reconnect(pClient);
			/* Network reconnect attempted, check if yield timer expired before
			 * doing anything else */
			continue;
		}

		yieldRc = zlg_iot_mqtt_internal_cycle_read(pClient, &timer, &packet_type);
		if(SUCCESS == yieldRc) {
			yieldRc = _zlg_iot_mqtt_keep_alive(pClient);
		} else {
			...
			if(NETWORK_SSL_READ_ERROR == yieldRc || NETWORK_SSL_READ_TIMEOUT_ERROR == yieldRc
				...
				yieldRc = _zlg_iot_mqtt_handle_disconnect(pClient);
			}
		}
		...
		if(NETWORK_DISCONNECTED_ERROR == yieldRc) {
			...
			if(1 == pClient->clientStatus.isAutoReconnectEnabled) {
				yieldRc = zlg_iot_mqtt_set_client_state(pClient, CLIENT_STATE_DISCONNECTED_ERROR,
														CLIENT_STATE_PENDING_RECONNECT);
				if(SUCCESS != yieldRc) {
					FUNC_EXIT_RC(yieldRc);
				}

				pClient->clientData.currentReconnectWaitInterval = ZLG_IOT_MQTT_MIN_RECONNECT_WAIT_INTERVAL;
				countdown_ms(&(pClient->reconnectDelayTimer), pClient->clientData.currentReconnectWaitInterval);
 ...
 *         iot_is_mqtt_connected can be called to confirm.
 */
IoT_Error_t zlg_iot_mqtt_yield(ZLG_IoT_Client *pClient, uint32_t timeout_ms) {
	...
	yieldRc = _zlg_iot_mqtt_internal_yield(pClient, timeout_ms);

	if(NETWORK_DISCONNECTED_ERROR != yieldRc && NETWORK_ATTEMPTING_RECONNECT != yieldRc) {
		rc = zlg_iot_mqtt_set_client_state(pClient, CLIENT_STATE_CONNECTED_YIELD_IN_PROGRESS,
...
static IoT_Error_t _zlg_iot_mqtt_internal_yield_once(ZLG_IoT_Client *pClient, uint32_t timeout_ms) {
	IoT_Error_t yieldRc = SUCCESS;
...
IoT_Error_t zlg_iot_mqtt_yield_once(ZLG_IoT_Client *pClient) {
	IoT_Error_t rc, yieldRc;
	ClientState clientState;
  ...
  uint32_t timeout_ms = 10 * 1000;
	...
	yieldRc = _zlg_iot_mqtt_internal_yield_once(pClient, timeout_ms);
